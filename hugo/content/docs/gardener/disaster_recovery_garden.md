...
path_base_for_github_subdir:
  from: content/docs/gardener/disaster_recovery_garden.md
  to: disaster_recovery_garden.md
persona: Operators
title: Disaster Recovery Garden
prev: false
next: false
---
<!-- BANNER:MANAGED -->
<!--
   ...
   └───────────│────────────────────────────────────┘
               ▼               
   https://github.com/gardener/gardener/blob/master/docs/operations/disaster_recovery_garden.md
-->
...
Reference:
- [Backup and Restore Concept](/docs/gardener/concepts/backup-restore/)
- [Garden ETCD](/docs/gardener/concepts/operator/#etcd)
...
Deploy the `gardener-operator` into the new runtime cluster as described [here](/docs/gardener/concepts/operator/#deployment).
...
1. **Delete Webhooks:** Delete the [mutating](/docs/gardener/concepts/operator/#defaulting) and [validating](/docs/gardener/concepts/operator/#validation) webhooks registered for the `Garden` resource to unblock later operations. Both webhooks are named `gardener-operator`. Once the operator is scaled up, it will recreate them.
1. **Deploy State Secrets:** Deploy the correct backed-up secrets for CAs, encryption, and signing keys into the new cluster.
1. **Deploy Infrastructure Secrets:** Deploy valid infrastructure credentials (e.g., for DNS and `etcd` backup) into the new cluster.
1. **Apply `Garden` Resource:** Apply the backed-up **`Garden` resource** to the new cluster.
...
However, there might be cases where the [backup bucket is configured with immutability](/docs/gardener/immutable-backup-buckets/).
...
Disaster Recovery can be tested with the [local development setup](/contribute/developer-starter-kit/local_setup/).
This may help to gain confidence before performing the procedure on production clusters.

### Preparation

For testing purposes, create a local `kind` cluster and deploy a Garden cluster into it by following the [Local Development Setup Guide](/docs/gardener/deployment/getting_started_locally/).
