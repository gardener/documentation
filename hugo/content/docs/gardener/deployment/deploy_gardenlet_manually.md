...
path_base_for_github_subdir:
  from: content/docs/gardener/deployment/deploy_gardenlet_manually.md
  to: deploy_gardenlet_manually.md
title: Deploy Gardenlet Manually
prev: false
next: false
managed: true
---
...
Manually deploying a gardenlet is usually only required if the Kubernetes cluster to be registered as a seed cluster is managed via third-party tooling (i.e., the Kubernetes cluster is not a shoot cluster, so [Deploy a gardenlet Automatically](/docs/gardener/deployment/deploy_gardenlet_automatically/) cannot be used).
In this case, `gardenlet` needs to be deployed manually, meaning that its [Helm chart](https://github.com/gardener/gardener/tree/master/charts/gardener/gardenlet) must be installed.
...
> For more information, see [Deploy a gardenlet Automatically](/docs/gardener/deployment/deploy_gardenlet_automatically/).

## Prerequisites

### Kubernetes Cluster that Should Be Registered as a Seed Cluster

- Verify that the cluster has a [supported Kubernetes version](/docs/gardener/shoot-operations/supported_k8s_versions/).
...
- Every seed cluster needs an Ingress controller which distributes external requests to internal components like Plutono and Prometheus.
  For this, configure the following lines in your [Seed resource](https://github.com/gardener/gardener/blob/master/example/50-seed.yaml):
  ```yaml
  spec:
    dns:
      provider:
...
The gardenlet needs to talk to the [Gardener API server](/docs/gardener/concepts/apiserver/) residing in the garden cluster.
...
For an overview over all values, see the [configuration values](https://github.com/gardener/gardener/blob/master/charts/gardener/gardenlet/values.yaml).
...
1. Create a bootstrap `kubeconfig` based on the bootstrap token created in the garden cluster.
...
1. In the `gardenClientConnection.bootstrapKubeconfig` section of your gardenlet configuration, provide the bootstrap `kubeconfig` together with a name and namespace to the gardenlet Helm chart.
...
1. In the `gardenClientConnection.kubeconfigSecret` section of your gardenlet configuration, define a name and a namespace where the gardenlet stores the real `kubeconfig` that it creates during the bootstrap process.
...
> - [Example Seed resource](https://github.com/gardener/gardener/blob/master/example/50-seed.yaml)
> - [Configurable Seed settings](/docs/gardener/seed_settings/)
...
1. Add the `seedConfig` to your gardenlet configuration `gardenlet-values.yaml`.
   ...
   For all supported infrastructure providers, see [Known Extension Implementations](/docs/extensions/#known-extension-implementations).
...
You may consider running `gardenlet` with multiple replicas, especially if the seed cluster is configured to host [HA shoot control planes](/docs/guides/high-availability/control-plane/).
...
1. Check that the bootstrap secret `gardenlet-bootstrap-kubeconfig` has been deleted from the seed cluster in namespace `garden`.

1. Check that the seed cluster is registered and `READY` in the garden cluster.
   
   Check that the seed cluster `sweet-seed` exists and all conditions indicate that it's available.
   If so, the [Gardenlet is sending regular heartbeats](/docs/gardener/concepts/gardenlet/#heartbeats) and the [seed bootstrapping](/docs/gardener/seed_bootstrapping/) was successful.
...
gardenlet runs [a controller](/docs/gardener/concepts/gardenlet/#gardenlet-controller) which watches for `seedmanagement.gardener.cloud/v1alpha1.Gardenlet` resources in the garden cluster in the `garden` namespace having the same name as the `Seed` the gardenlet is responsible for.
...
1. The bootstrap secret `gardenlet-bootstrap-kubeconfig` has been deleted automatically:
   ```bash
   kubectl -n garden get secret gardenlet-bootstrap-kubeconfig
   ```

1. The `Seed` resource in the garden cluster shows healthy conditions:
   ```bash
...
- [Backup and Restore](/docs/gardener/concepts/backup-restore/).
