...
path_base_for_github_subdir:
  from: content/docs/gardener/deployment/gardenlet_api_access.md
  to: gardenlet_api_access.md
...
<!--
   ...
   └───────────│────────────────────────────────────┘
               ▼               
   https://github.com/gardener/gardener/blob/master/docs/deployment/gardenlet_api_access.md
-->
...
Starting from Gardener [`v1.74.0`](https://github.com/gardener/gardener/releases/v1.74.0), extensions running on seed clusters can also get [access to the garden cluster](/docs/gardener/extensions/garden-api-access/) using a token for a dedicated ServiceAccount.
...
![Flow Diagram](/docs/gardener/deployment/content/gardenlet_api_access_flow.png)
...
| `ConfigMap` | `get` | `ConfigMap` -> `Seed`, `ConfigMap` -> `Shoot` -> `Seed`, `ConfigMap` -> `ControllerDeployment` -> `ControllerInstallation` -> `Seed` | Allow only `get` requests for `ConfigMap`s referenced by `Seed`s, `Shoot`s assigned to the `gardenlet`'s `Seed`, or by `ControllerDeployment`s of `ControllerInstallation`s assigned to the `gardenlet`'s `Seed`. Allow reading the `kube-system/cluster-identity` `ConfigMap`. |
...
| `InternalSecret` | `create`, `get`, `list`, `watch`, `patch`, `update`, `delete` | `InternalSecret` -> `CredentialsBinding` -> `Shoot` -> `Seed`, `InternalSecret` -> `Shoot` -> `Seed` | Allow read requests for `InternalSecret`s referenced by `CredentialsBinding`s referenced by `Shoot`s that are assigned to the `gardenlet`'s `Seed`. Also allow write requests for `InternalSecret`s belonging to the `Shoot` directly (e.g., client CA secret). |
...
| `Secret` | `create`, `get`, `update`, `patch`, `delete`(, `list`, `watch`) | `Secret` -> `Seed`, `Secret` -> `Shoot` -> `Seed`, `Secret` -> `SecretBinding` -> `Shoot` -> `Seed`, `Secret` -> `CredentialsBinding` -> `Shoot` -> `Seed`, `Secret` -> `BackupBucket` -> `Seed`, `Secret` -> `ControllerDeployment` -> `ControllerInstallation` -> `Seed` | Allow `get`, `list`, `watch` requests for all `Secret`s in the `seed-<name>` namespace. Allow only `create`, `get`, `update`, `patch`, `delete` requests for the `Secret`s related to resources assigned to the `gardenlet`'s `Seed`s. |
...
1. If the authenticated user belongs to the `gardener.cloud:system:seeds` group, it is considered a gardenlet client.
   - This requires a proper TLS certificate that the `gardenlet` uses to contact the API server and is automatically given if [TLS bootstrapping](/docs/gardener/concepts/gardenlet/#TLS-Bootstrapping) is used.
...
1. If the authenticated user belongs to the `system:serviceaccounts` group, it is considered an extension client under the following conditions:
...
![Resource Dependency Graph](/docs/gardener/deployment/content/gardenlet_api_access_graph.png)
