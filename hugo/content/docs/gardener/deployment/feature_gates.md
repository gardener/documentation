...
<!--
   ...
   └───────────│────────────────────────────────────┘
               ▼               
   https://github.com/gardener/gardener/blob/master/docs/deployment/feature_gates.md
-->
...
Each Gardener component lets you enable or disable a set of feature gates that are relevant to that component. For example, this is the configuration of the [gardenlet](https://github.com/gardener/gardener/blob/master/example/20-componentconfig-gardenlet.yaml) component. [Configure feature gates](/docs/gardener/deployment/getting_started_locally/#configure-feature-gates-optional) provides instructions on configuring feature gates for different Gardener components using the local Gardener setup as an example.
...
| LiveControlPlaneMigration | `gardener-apiserver`, `gardenlet` | Enables setting the `migration.gardener.cloud/live-migrate=true` annotation on a Shoot together with a `spec.seedName` change via the `shoots/binding` subresource to trigger live control-plane migration. On the `gardenlet` side it drives the additional reconciliation steps for the migration (e.g. cross-seed etcd peer/client exposure via the shared Istio ingress gateway and the corresponding ports/network policies). See [Live Control Plane Migration](/docs/gardener/live_control_plane_migration/) and [GEP-0039](https://github.com/gardener/enhancements/tree/main/geps/0039-live-control-plane-migration). |
