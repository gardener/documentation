...
path_base_for_github_subdir:
  from: content/docs/gardener/istio.md
  to: istio.md
persona: Operators
title: Istio
prev: false
...
<!--
   ...
   └───────────│────────────────────────────────────┘
               ▼               
   https://github.com/gardener/gardener/blob/master/docs/operations/istio.md
-->
...
![Multi Availability Zone Handling in Istio](/docs/gardener/images/multi-zonal-istio.png)

Please note that operators may need to perform additional tuning to prevent cross-zonal traffic completely. The [loadbalancer settings in the seed specification](/docs/gardener/seed_settings/#load-balancer-services) offer various options, e.g., by setting the external traffic policy to `local` or using infrastructure specific loadbalancer annotations.

Furthermore, note that this approach is also taken in case [`ExposureClass`es](/docs/gardener/networking/exposureclasses/) are used. For each exposure class, additional zonal istio ingress gateways may be deployed to cover for single-zone shoot control planes using the exposure class.
...
For more details on how to disable zonal istio ingress gateways, see the [seed settings documentation](/docs/gardener/seed_settings/#zonal-ingress).
