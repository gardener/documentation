...
path_base_for_github_subdir:
  from: content/docs/gardener/concepts/controller-manager.md
  to: controller-manager.md
...
<!--
   ...
   └───────────│────────────────────────────────────┘
               ▼               
   https://github.com/gardener/gardener/blob/master/docs/concepts/controller-manager.md
-->
...
### [`Bastion` Controller](https://github.com/gardener/gardener/tree/master/pkg/controllermanager/controller/bastion)
...
The `maxLifetime` defaults to 24 hours and is an option in the `BastionControllerConfiguration` which is part of `gardener-controller-manager`s `ControllerManagerControllerConfiguration`, see [the example config file](https://github.com/gardener/gardener/blob/master/example/20-componentconfig-gardener-controller-manager.yaml) for details.
...
### [`CertificateSigningRequest` Controller](https://github.com/gardener/gardener/tree/master/pkg/controllermanager/controller/certificatesigningrequest)

After the [gardenlet](/docs/gardener/concepts/gardenlet/) gets deployed on the Seed cluster, it needs to establish itself as a trusted party to communicate with the Gardener API server. It runs through a bootstrap flow similar to the [kubelet bootstrap](https://kubernetes.io/docs/reference/access-authn-authz/kubelet-tls-bootstrapping/) process.
...
1. The requesting client has permission to create the `certificatesigningrequests/seedclient` subresource
...
1. The requesting client must have permission to create the `certificatesigningrequests/shootclient` subresource
1. **Bootstrap Token Validation**: The CSR must be requested via a bootstrap token with a specific description format containing the shoot's namespace and name
1. **Subject Matching**: The certificate request's subject must match the shoot metadata extracted from the bootstrap token description
...
### [`CloudProfile` Controller](https://github.com/gardener/gardener/tree/master/pkg/controllermanager/controller/cloudprofile)
...
### [`NamespacedCloudProfile` Controller](https://github.com/gardener/gardener/tree/master/pkg/controllermanager/controller/namespacedcloudprofile)
...
### [`ControllerDeployment` Controller](https://github.com/gardener/gardener/tree/master/pkg/controllermanager/controller/controllerdeployment)

Extensions are registered in the garden cluster via `ControllerRegistration` and deployment of respective extensions are specified via `ControllerDeployment`. For more info refer to [Registering Extension Controllers](/docs/gardener/extensions/registration/).

The controller has the following reconcilers:

#### ["Main" Reconciler](https://github.com/gardener/gardener/tree/master/pkg/controllermanager/controller/controllerdeployment/controllerdeployment)
...
#### ["Reference" Reconciler](https://github.com/gardener/gardener/tree/master/pkg/controllermanager/controller/controllerdeployment/reference)

`ControllerDeployment` objects may reference `Secret`s and `ConfigMap`s in the `garden` namespace of the garden cluster via `.resources[]` to inject their values into the Helm chart values used to deploy the extension (see [Referencing Resources in Helm Values](/docs/gardener/extensions/registration/#referencing-resources-in-helm-values)).
...
### [`ControllerRegistration` Controller](https://github.com/gardener/gardener/tree/master/pkg/controllermanager/controller/controllerregistration)

The `ControllerRegistration` controller makes sure that the required [Gardener Extensions](/docs/gardener/#extensions) specified by the [`ControllerRegistration`](/docs/gardener/extensions/registration/#controllerregistrations) resources are present in the seed clusters.
...
#### ["Seed" Reconciler](https://github.com/gardener/gardener/tree/master/pkg/controllermanager/controller/controllerregistration/controllerinstallation/seed)
...
For the configuration options, please see this [section](/docs/gardener/extensions/registration/#deployment-configuration-options).
...
#### ["Self-Hosted Shoot" Reconciler](https://github.com/gardener/gardener/tree/master/pkg/controllermanager/controller/controllerregistration/controllerinstallation/shoot)
...
#### ["`ControllerRegistration` Finalizer" Reconciler](https://github.com/gardener/gardener/tree/master/pkg/controllermanager/controller/controllerregistration/controllerregistrationfinalizer)
...
#### ["Cluster Finalizer" Reconciler](https://github.com/gardener/gardener/tree/master/pkg/controllermanager/controller/controllerregistration/clusterfinalizer)
...
#### ["Extension `ClusterRole`" Reconciler](https://github.com/gardener/gardener/tree/master/pkg/controllermanager/controller/controllerregistration/extensionclusterrole)
...
You can read more about the purpose of this reconciler in [this document](/docs/gardener/extensions/garden-api-access/#additional-permissions).

### [`CredentialsBinding` Controller](https://github.com/gardener/gardener/tree/master/pkg/controllermanager/controller/credentialsbinding)
...
### [`Event` Controller](https://github.com/gardener/gardener/tree/master/pkg/controllermanager/controller/event)
...
### [`ExposureClass` Controller](https://github.com/gardener/gardener/tree/master/pkg/controllermanager/controller/exposureclass)

`ExposureClass` abstracts the ability to expose a Shoot clusters control plane in certain network environments (e.g. corporate networks, DMZ, internet) on all Seeds or a subset of the Seeds. For more information, see [ExposureClasses](/docs/gardener/networking/exposureclasses/).
...
### [Gardenlet Lifecycle Controller](https://github.com/gardener/gardener/tree/master/pkg/controllermanager/controller/gardenletlifecycle)
...
### [`Quota` Controller](https://github.com/gardener/gardener/tree/master/pkg/controllermanager/controller/quota)
...
### [`Project` Controller](https://github.com/gardener/gardener/tree/master/pkg/controllermanager/controller/project)

There are multiple controllers responsible for different aspects of `Project` objects.
Please also refer to the [`Project` documentation](/docs/gardener/project/projects/).

#### ["Main" Reconciler](https://github.com/gardener/gardener/tree/master/pkg/controllermanager/controller/project/project)
...
Gardener administrators and extension developers can define their own roles. For more information, see [Extending Project Roles](/docs/gardener/extensions/project-roles/) for more information.
...
#### ["Stale Projects" Reconciler](https://github.com/gardener/gardener/tree/master/pkg/controllermanager/controller/project/stale)
...
#### ["Activity" Reconciler](https://github.com/gardener/gardener/tree/master/pkg/controllermanager/controller/project/activity)
...
#### [`ResourceQuota` Reconciler](https://github.com/gardener/gardener/tree/master/pkg/controllermanager/controller/project/resourcequota)
...
### [`SecretBinding` Controller](https://github.com/gardener/gardener/tree/master/pkg/controllermanager/controller/secretbinding)
...
### [`Seed` Controller](https://github.com/gardener/gardener/tree/master/pkg/controllermanager/controller/seed)
...
#### ["Main" Reconciler](https://github.com/gardener/gardener/tree/master/pkg/controllermanager/controller/seed/secrets)
...
#### ["Backup Buckets Check" Reconciler](https://github.com/gardener/gardener/tree/master/pkg/controllermanager/controller/seed/backupbucketscheck)
...
#### ["Reference" Reconciler](https://github.com/gardener/gardener/tree/master/pkg/controllermanager/controller/seed/reference)
...
### [`Shoot` Controller](https://github.com/gardener/gardener/tree/master/pkg/controllermanager/controller/shoot)

#### ["Conditions" Reconciler](https://github.com/gardener/gardener/tree/master/pkg/controllermanager/controller/shoot/conditions)
...
#### ["Hibernation" Reconciler](https://github.com/gardener/gardener/tree/master/pkg/controllermanager/controller/shoot/hibernation)
...
It ignores [failed `Shoot`s](/docs/gardener/shoot/shoot_status/#last-operation) and those marked for deletion.

#### ["Maintenance" Reconciler](https://github.com/gardener/gardener/tree/master/pkg/controllermanager/controller/shoot/maintenance)
...
It could also add some operation or task annotations. For more information, see [Shoot Maintenance](/docs/gardener/shoot/shoot_maintenance/).

#### ["Quota" Reconciler](https://github.com/gardener/gardener/tree/master/pkg/controllermanager/controller/shoot/quota)
...
#### ["Reference" Reconciler](https://github.com/gardener/gardener/tree/master/pkg/controllermanager/controller/shoot/reference)
...
#### ["Retry" Reconciler](https://github.com/gardener/gardener/tree/master/pkg/controllermanager/controller/shoot/retry)

This reconciler is responsible for retrying certain failed `Shoot`s.
Currently, the reconciler retries only failed `Shoot`s with an error code `ERR_INFRA_RATE_LIMITS_EXCEEDED`. See [Shoot Status](/docs/gardener/shoot/shoot_status/#error-codes) for more details.

#### ["Status Label" Reconciler](https://github.com/gardener/gardener/tree/master/pkg/controllermanager/controller/shoot/statuslabel)

This reconciler is responsible for maintaining the `shoot.gardener.cloud/status` label on `Shoot`s. See [Shoot Status](/docs/gardener/shoot/shoot_status/#status-label) for more details.

#### ["Migration" Reconciler](https://github.com/gardener/gardener/tree/master/pkg/controllermanager/controller/shoot/migration)
...
#### ["ShootState Finalizer" Reconciler](https://github.com/gardener/gardener/tree/master/pkg/controllermanager/controller/shootstate)
