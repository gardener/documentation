---
description: >-
  Set of controllers with different responsibilities running once per seed and
  once per shoot
...
path_base_for_github_subdir:
  from: content/docs/gardener/concepts/resource-manager.md
  to: resource-manager.md
...
<!--
   ...
   └───────────│────────────────────────────────────┘
               ▼               
   https://github.com/gardener/gardener/blob/master/docs/concepts/resource-manager.md
-->
...
You can find an example configuration file [here](https://github.com/gardener/gardener/blob/master/example/resource-manager/10-componentconfig.yaml).

## Controllers

### [`ManagedResource` Controller](https://github.com/gardener/gardener/tree/master/pkg/resourcemanager/controller/managedresource)
...
1. Creating all referenced resources by the Gardener Resource Manager that is responsible for the new class in its target cluster.

#### [Conditions](https://github.com/gardener/gardener/tree/master/pkg/resourcemanager/controller/health)
...
### [`health` Controller](https://github.com/gardener/gardener/tree/master/pkg/resourcemanager/controller/health)
...
### [Garbage Collector For Immutable `ConfigMap`s/`Secret`s](https://github.com/gardener/gardener/tree/master/pkg/resourcemanager/controller/garbagecollector)
...
### [TokenRequestor Controller](https://github.com/gardener/gardener/tree/master/pkg/controller/tokenrequestor)
...
See [this YAML file](https://github.com/gardener/gardener/blob/master/example/resource-manager/30-secret-tokenrequestor.yaml) for an example of the secret.
...
![image](/docs/gardener/concepts/images/resource-manager-projected-token-controlplane-to-shoot-apiserver.jpg)
...
### [`CertificateSigningRequest` Approver](https://github.com/gardener/gardener/tree/master/pkg/resourcemanager/controller/csrapprover)
...
1. server certificate for serving its HTTPS server
...
There is a second use case for `CSR Approver`, because [Gardener Node Agent](/docs/gardener/concepts/node-agent/) is able to use client certificates for communication with `kube-apiserver`.
...
### [`NetworkPolicy` Controller](https://github.com/gardener/gardener/tree/master/pkg/resourcemanager/controller/networkpolicy)
...
### [`IstioClusterConfiguration` Controller](https://github.com/gardener/gardener/tree/master/pkg/resourcemanager/controller/istioclusterconfiguration)
...
### [`Node` Controller](https://github.com/gardener/gardener/tree/master/pkg/resourcemanager/controller/node)

#### [Critical Components Controller](https://github.com/gardener/gardener/tree/master/pkg/resourcemanager/controller/node/criticalcomponents)
...
Please refer to the [feature documentation](/docs/gardener/advanced/node-readiness/) or [proposal issue](https://github.com/gardener/gardener/issues/7117) for more details.

#### [Node Agent Reconciliation Delay Controller](https://github.com/gardener/gardener/tree/master/pkg/resourcemanager/controller/node/agentreconciliationdelay)
...
The controller adds the `node-agent.gardener.cloud/reconciliation-delay` annotation to nodes whose value is read by the [node-agent](/docs/gardener/concepts/node-agent/)s.
...
> Read more about it [here](/docs/gardener/concepts/node-agent/#serial-reconciliation).

#### [High Availability Config Controller](https://github.com/gardener/gardener/tree/master/pkg/resourcemanager/controller/node/highavailabilityconfig)
...
The details and scenarios are described in [High Availability Of Deployed Components](/contribute/gardener/high-availability-of-components/).
...
1. [Topology Spread Constraints](https://kubernetes.io/docs/concepts/scheduling-eviction/topology-spread-constraints/) are added to the pod template spec when the `.spec.replicas` are greater than `1`. When the `high-availability-config.resources.gardener.cloud/zones` annotation ...
...
1. Adds default tolerations for [taint-based evictions](https://kubernetes.io/docs/concepts/scheduling-eviction/taint-and-toleration/#taint-based-evictions):
   ...
   The `TolerationSeconds` are taken from the respective configuration section of the webhook's configuration (see [example](https://github.com/gardener/gardener/blob/master/example/resource-manager/10-componentconfig.yaml))).
   
   We consider fine-tuned values for those tolerations a matter of high-availability because they often help to reduce recovery times in case of node or zone outages, also see [High-Availability Best Practices](/docs/guides/high-availability/best-practices/).
...
1. The `Pod`'s `.spec.serviceAccountName` field is NOT empty and NOT set to `default`.
1. The `ServiceAccount` specified in the `Pod`'s `.spec.serviceAccountName` sets `.automountServiceAccountToken=false`.
1. The `Pod`'s `.spec.volumes[]` DO NOT already contain a volume with a name prefixed with `kube-api-access-`.
...
![image](/docs/gardener/concepts/images/resource-manager-projected-token-shoot-to-shoot-apiserver.jpg)
...
1. Add `pod.spec.tolerations` as given in the webhook configuration.
1. Add `pod.spec.tolerations` for any existing nodes matching the node selector given in the webhook configuration. Known taints and tolerations used for [taint based evictions](https://kubernetes.io/docs/concepts/scheduling-eviction/taint-and-toleration/#taint-based-evictions) are disregarded.
...
Both is required to ensure system component `Pod`s can be *scheduled* or *executed* during an active shoot reconciliation that is happening due to any modifications to `shoot.spec.provider.workers[].taints`, e.g. `Pod`s must be scheduled while there are still `Node`s not having the updated taint configuration.
...
This webhook is used in the context of [L7 load balancing for kube-apiservers](/docs/gardener/kube_apiserver_loadbalancing/).
...
configured with `Auto` or `Recreate` to switch to update mode `InPlaceOrRecreate`, enabling *in-place* `Pod` [resources updates](https://kubernetes.io/blog/2025/05/16/kubernetes-v1-33-in-place-pod-resize-beta/). Preserving the `resource-manager` scope, the following constraints apply:
- *mutates* all `vpa` resources **inside** `kube-system` and `kubernetes-dashboards` namespaces on `Shoot` clusters
- *mutates* all `vpa` resources **outside** `kube-system` and `kubernetes-dashboards` namespaces on `Seed` clusters

In addition, to prevent the *mutation* of certain `VerticalPodAutoscaler` [resources](https://github.com/kubernetes/autoscaler/blob/master/vertical-pod-autoscaler/deploy/vpa-crd.yaml) falling within the webhook's scope, the following *label*

```
vpa-in-place-updates.resources.gardener.cloud/skip
```

could be appended to the resource metadata. With the *label* specified, the webhook will filter out the resources, leaving its current `updateMode` configuration.
To indicate that a `vpa` resource has already been mutated by the *webhook*, an additional

```
vpa-in-place-updates.resources.gardener.cloud/mutated
```

*label* gets added to the resource metadata.

The webhook is enabled unconditionally for resource manager deployments managed by both [gardenlet](/docs/gardener/concepts/gardenlet/) and [gardener operator](/docs/gardener/concepts/operator/).

### Validating Webhooks

#### Unconfirmed Deletion Prevention For Custom Resources And Definitions

As part of Gardener's [extensibility concepts](/docs/gardener/extensions/), a lot of `CustomResourceDefinition`s are deployed to the seed clusters that serve as extension points for provider-specific controllers.
For example, the [`Infrastructure` CRD](/docs/gardener/extensions/resources/infrastructure/) triggers the provider extension to prepare the IaaS infrastructure of the underlying cloud provider for a to-be-created shoot cluster.
...
It works similar to [`SeedAuthorizer`](/docs/gardener/deployment/gardenlet_api_access/). However, the logic used to make decisions is much simpler so it does not implement a decision graph.
