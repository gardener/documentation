---
description: >-
  Understand how the gardenlet, the primary "agent" on every seed cluster, works
  and learn more about the different Gardener components
github_repo: 'https://github.com/gardener/gardener'
github_subdir: docs/concepts
params:
  github_branch: master
path_base_for_github_subdir:
  from: content/docs/gardener/concepts/gardenlet.md
  to: gardenlet.md
title: gardenlet
prev: false
next: false
...
<!--
   ...
   └───────────│────────────────────────────────────┘
               ▼               
   https://github.com/gardener/gardener/blob/master/docs/concepts/gardenlet.md
-->
...
![Counterparts in the Gardener Architecture and the Kubernetes Architecture](/docs/gardener/concepts/images/gardenlet-architecture-similarities.png)
...
![Reversed Control Flow Using a gardenlet](/docs/gardener/concepts/images/gardenlet-architecture-detailed.png)
  ...
  spec:
    ...
    settings:
      scheduling:
        visible: true
```
(see [this yaml file](https://github.com/gardener/gardener/blob/master/example/20-componentconfig-gardenlet.yaml) for a more complete example)
...
### [`BackupBucket` Controller](https://github.com/gardener/gardener/tree/master/pkg/gardenlet/controller/backupbucket)
...
### [`BackupEntry` Controller](https://github.com/gardener/gardener/tree/master/pkg/gardenlet/controller/backupentry)
...
The controller creates an `extensions.gardener.cloud/v1alpha1.BackupEntry` resource (non-namespaced) in the seed cluster and waits until the responsible extension controller reconciled it (see [Contract: BackupEntry Resource](/docs/gardener/extensions/resources/backupentry/) for more details).
...
Additionally, you can limit the [shoot purposes](/docs/gardener/shoot/shoot_purposes/) for which this applies by setting `.controllers.backupEntry.deletionGracePeriodShootPurposes[]`.
...
### [`Bastion` Controller](https://github.com/gardener/gardener/tree/master/pkg/gardenlet/controller/bastion)
...
### [`ControllerInstallation` Controller](https://github.com/gardener/gardener/tree/master/pkg/gardenlet/controller/controllerinstallation)
...
#### ["Main" Reconciler](https://github.com/gardener/gardener/tree/master/pkg/gardenlet/controller/controllerinstallation/controllerinstallation)
...
Then, it creates a generic garden kubeconfig and garden access secret for the extension for [accessing the garden cluster](/docs/gardener/extensions/garden-api-access/).
...
#### ["Care" Reconciler](https://github.com/gardener/gardener/tree/master/pkg/gardenlet/controller/controllerinstallation/care)
...
#### ["Required" Reconciler](https://github.com/gardener/gardener/tree/master/pkg/gardenlet/controller/controllerinstallation/required)
...
This condition is taken into account by the `ControllerRegistration` controller part of `gardener-controller-manager` when it computes which extensions have to be deployed to which seed cluster. See [Gardener Controller Manager](/docs/gardener/concepts/controller-manager/#controllerregistration-controller) for more details.

### [`Gardenlet` Controller](https://github.com/gardener/gardener/tree/master/pkg/gardenlet/controller/gardenlet)
...
### [`ManagedSeed` Controller](https://github.com/gardener/gardener/tree/master/pkg/gardenlet/controller/managedseed)
...
### [`NetworkPolicy` Controller](https://github.com/gardener/gardener/tree/master/pkg/gardenlet/controller/networkpolicy)
...
For more details about `NetworkPolicy`s in Gardener, please see [`NetworkPolicy`s In Garden, Seed, Shoot Clusters](/docs/gardener/network_policies/).

### [`Seed` Controller](https://github.com/gardener/gardener/tree/master/pkg/gardenlet/controller/seed)

The `Seed` controller in the `gardenlet` reconciles `Seed` objects with the help of the following reconcilers.

#### ["Main Reconciler"](https://github.com/gardener/gardener/tree/master/pkg/gardenlet/controller/seed/seed)
...
It also checks whether the seed cluster's Kubernetes version is at least the [minimum supported version](/docs/gardener/shoot-operations/supported_k8s_versions/#seed-clusters) and errors in case this constraint is not met.
...
#### ["Care" Reconciler](https://github.com/gardener/gardener/tree/master/pkg/gardenlet/controller/seed/care)
...
#### ["Lease" Reconciler](https://github.com/gardener/gardener/tree/master/pkg/gardenlet/controller/seed/lease)
...
### [`Shoot` Controller](https://github.com/gardener/gardener/tree/master/pkg/gardenlet/controller/shoot)

The `Shoot` controller in the `gardenlet` reconciles `Shoot` objects with the help of the following reconcilers.

#### ["Main" Reconciler](https://github.com/gardener/gardener/tree/master/pkg/gardenlet/controller/shoot/shoot)
...
It is also responsible for syncing the [`Cluster` cluster](/docs/gardener/extensions/cluster/) to the seed cluster before and after each successful shoot reconciliation.

The main reconciliation logic is performed in 3 different task flows dedicated to specific operation types:

- `reconcile` (operations: create, reconcile, restore): this is the main flow responsible for creation and regular reconciliation of shoots. Hibernating a shoot also triggers this flow. It is also used for restoration of the shoot control plane on the new seed (second half of a [Control Plane Migration](/docs/gardener/control_plane_migration/#shoot-control-plane-migration))
- `migrate`: this flow is triggered when `spec.seedName` specifies a different seed than `status.seedName`. It performs the first half of the [Control Plane Migration](/docs/gardener/control_plane_migration/#shoot-control-plane-migration), i.e., a backup (`migrate` operation) of all control plane components followed by a "shallow delete".
...
- If `status.observedGeneration` is less than `metadata.generation`: this is the case, e.g., when the spec was changed, a [manual reconciliation operation](/docs/gardener/shoot-operations/shoot_operations/) was triggered, or the shoot was deleted.
- If the [last operation](/docs/gardener/shoot/shoot_status/) was not successful.
- If the shoot is in a [failed state](/docs/gardener/shoot/shoot_status/), the gardenlet does not perform any reconciliation on the shoot (unless the retry operation was triggered). However, it syncs the `Cluster` resource to the seed in order to inform the extension controllers about the failed state.
...
- In case `GardenletConfiguration.controllers.shoot.reconcileInMaintenanceOnly` is enabled (disabled by default), the gardenlet performs regular shoot reconciliations only once in the respective maintenance time window (`GardenletConfiguration.controllers.shoot.syncPeriod` is ignored). The gardenlet randomly distributes shoot reconciliations over the maintenance time window to avoid high bursts of reconciliations (see [Shoot Maintenance](/docs/gardener/shoot/shoot_maintenance/#cluster-reconciliation)).
- In case `Shoot.spec.maintenance.confineSpecUpdateRollout` is enabled (disabled by default), changes to the shoot specification are not rolled out immediately but only during the respective maintenance time window (see [Shoot Maintenance](/docs/gardener/shoot/shoot_maintenance/)).

#### ["Care" Reconciler](https://github.com/gardener/gardener/tree/master/pkg/gardenlet/controller/shoot/care)
...
Each condition can optionally also have error `codes` in order to indicate which type of issue was detected (see [Shoot Status](/docs/gardener/shoot/shoot_status/) for more details).

Apart from the above, extension controllers can also contribute to the `status` or error `codes` of these conditions (see [Contributing to Shoot Health Status Conditions](/docs/gardener/extensions/shoot-health-status-conditions/) for more details).
...
Besides directly checking the status of `Deployment`s, `Etcd`s, `StatefulSet`s in the shoot namespace, this reconciler also considers `ManagedResource`s (in the shoot namespace) and their status in order to compute the condition statuses, see [this document](/docs/gardener/concepts/resource-manager/#conditions) for more information.
The following table explains which `ManagedResource`s are considered for which condition type:
...
Please see [Shoot Status](/docs/gardener/shoot/shoot_status/#constraints) for more details.
...
#### ["Lease" Reconciler](https://github.com/gardener/gardener/tree/master/pkg/gardenlet/controller/shoot/lease)
...
#### ["State" Reconciler](https://github.com/gardener/gardener/tree/master/pkg/gardenlet/controller/shoot/state)
...
### ["Status" Reconciler](https://github.com/gardener/gardener/tree/master/pkg/gardenlet/controller/shoot/status)
...
### [`TokenRequestor` Controller For `WorkloadIdentity`s](https://github.com/gardener/gardener/tree/master/pkg/gardenlet/controller/tokenrequestor/workloadidentity)
...
### [`VPAEvictionRequirements` Controller](https://github.com/gardener/gardener/tree/master/pkg/gardenlet/controller/vpaevictionrequirements)
...
For more information, see [`ManagedSeed`s: Register Shoot as Seed](/docs/gardener/managed_seed/).
...
See [Deploy a gardenlet](/docs/gardener/deployment/deploy_gardenlet/) for all instructions.

## Related Links

- [Gardener Architecture](/docs/gardener/concepts/architecture/)
