---
description: >-
  Understand the component responsible for the garden cluster environment and
...
path_base_for_github_subdir:
  from: content/docs/gardener/concepts/operator.md
  to: operator.md
...
<!--
   ...
   └───────────│────────────────────────────────────┘
               ▼               
   https://github.com/gardener/gardener/blob/master/docs/concepts/operator.md
-->
...
There is a [Helm chart](https://github.com/gardener/gardener/tree/master/charts/gardener/operator) which can be used to deploy the `gardener-operator`.
...
Please find an exemplary `Garden` resource [here](https://github.com/gardener/gardener/blob/master/example/operator/20-garden.yaml).
...
For detailed information on external traffic policy, proxy protocol, and migration guidance, refer to the [Seed Settings - Load Balancer Services](/docs/gardener/seed_settings/#load-balancer-services) documentation. The settings work the same way for the Garden cluster, with the following difference:
...
Refer to the [Topology-Aware Traffic Routing documentation](/docs/gardener/topology_aware_routing/) as this document contains the documentation for the topology-aware routing setting for the garden runtime cluster.
...
You can find out more about Gardener extensions and how they can be used [here](/docs/gardener/extensions/resources/extension/#contract-extension-resource).
...
- The deployment of [extension admissions charts](/docs/gardener/extensions/admission/) in runtime and virtual clusters.
...
Please find an exemplary `Extension` resource [here](https://github.com/gardener/gardener/blob/master/example/operator/15-extension.yaml).
...
`gardener-operator` follows the same principles described by [this document](/docs/gardener/extensions/registration/):
...
- `.spec.deployment.extension.policy` and `.spec.deployment.extension.seedSelector` define the extension's installation policy as per the [`ControllerDeployment's` respective fields](/docs/gardener/extensions/registration/#deployment-configuration-options)
...
The configuration allows for precise control over various extension parameters, such as requested resources, [priority classes](/contribute/gardener/priority-classes/), and more.
...
When the virtual garden cluster is available, the `Extension` controller manages [`ControllerRegistration`/`ControllerDeployment` resources](/docs/gardener/extensions/registration/#controllerregistrations)
...
Therefore, Gardener automatically injects a kubeconfig into the admission deployment to interact with the (virtual) garden cluster (see [this document](/docs/gardener/extensions/garden-api-access/) for more information).
...
These include both well-known types such as `Infrastructure`, `Worker` etc. and [generic resources](/docs/gardener/extensions/registration/#extension-resource-configurations).
...
### [`Garden` Controller](https://github.com/gardener/gardener/tree/master/pkg/operator/controller/garden)

The Garden controller in the operator reconciles Garden objects with the help of the following reconcilers.

#### [`Main` Reconciler](https://github.com/gardener/gardener/tree/master/pkg/operator/controller/garden/garden)
...
The [`gardener-resource-manager`](/docs/gardener/concepts/resource-manager/) is deployed first since its `ManagedResource` controller will be used to bring up the remainders.

Other system components are:

- runtime garden system resources ([`PriorityClass`es](/contribute/gardener/priority-classes/) for the workload resources)
...
The whole management works similar to how it works for `Shoot`s, so you can take a look at [this document](/docs/gardener/concepts/etcd/) for more information in general.
...
This works similar like for `Shoot`s (see [this document](/docs/guides/high-availability/control-plane/)) except for the fact that there is no failure tolerance type configurability.
The `gardener-resource-manager`'s [HighAvailabilityConfig webhook](/docs/gardener/concepts/resource-manager/#high-availability-config) makes sure that all pods with multiple replicas are spread on nodes, and if there are at least two zones in `.spec.runtimeCluster.provider.zones` then they also get spread across availability zones.
...
Third-party components that need to communicate with the virtual cluster can leverage the [`gardener-resource-manager`'s `TokenRequestor` controller](/docs/gardener/concepts/resource-manager/#tokenrequestor-controller) and the generic kubeconfig, just like it works for `Shoot`s.
...
- `frontendConfigMapRef`: Reference a `ConfigMap` in the `garden` namespace containing the frontend configuration in the data with key `frontend-config.yaml`, for example
  ...
  Read more about it [here](/docs/dashboard/customization/).
...
- `assetsConfigMapRef`: Reference a `ConfigMap` in the `garden` namespace containing the assets, for example
  ...
  Please take a look at [this file](/docs/dashboard/customization/#logos-and-icons) to get more information.
...
- `terminal`: This enables the web terminal feature, read more about it [here](/docs/dashboard/webterminals/).
  ...
  The `allowedHosts` field is explained [here](/docs/dashboard/webterminals/#configuration).
...
- `ingress` (deprecated): This allows you to customize the `Ingress` resource of the dashboard.
  The `enabled` field allows you to control whether to deploy the `Ingress` resource to the cluster.
  - This field is deprecated and will be removed in a future version. Use the `domain` and `tlsSecretName` fields for customizing the ingress domain instead.
- `propagateCAFromSNI`: Controls whether the dashboard includes the CA certificate from the SNI TLS secret in its `apiServerCaData` configuration.
  All kubeconfigs generated by the dashboard (for terminal sessions, OIDC login, service account access) use this CA to trust the kube-apiserver.
...
#### [`Care` Reconciler](https://github.com/gardener/gardener/tree/master/pkg/operator/controller/garden/care)
...
In order to compute the condition statuses, this reconciler considers `ManagedResource`s (in the `garden` and `istio-system` namespace) and their status, see [this document](/docs/gardener/concepts/resource-manager/#conditions) for more information.
...
#### [`Reference` Reconciler](https://github.com/gardener/gardener/tree/master/pkg/operator/controller/garden/reference)
...
### [`Controller Registrar` Controller](https://github.com/gardener/gardener/tree/master/pkg/operator/controller/controllerregistrar)
...
* [`NetworkPolicy` controller](/docs/gardener/concepts/gardenlet/#networkpolicy-controller)
* [`VPA EvictionRequirements` controller](/docs/gardener/concepts/gardenlet/#vpaevictionrequirements-controller)
...
### [`Extension` Controller](https://github.com/gardener/gardener/tree/master/pkg/operator/controller/extension)
...
#### [`Extension Care` Reconciler](https://github.com/gardener/gardener/tree/master/pkg/operator/controller/extension/care)
...
#### [`Main` Reconciler](https://github.com/gardener/gardener/tree/master/pkg/operator/controller/extension/extension)
...
#### [`Required Runtime` Reconciler](https://github.com/gardener/gardener/tree/master/pkg/operator/controller/extension/required/runtime)
...
#### [`Required Virtual` Reconciler](https://github.com/gardener/gardener/tree/master/pkg/operator/controller/extension/required/virtual)
...
#### [`Reference` Reconciler](https://github.com/gardener/gardener/tree/master/pkg/operator/controller/extension/reference)

`Extension` objects may reference `Secret`s and `ConfigMap`s in the `garden` namespace of the runtime cluster via `.spec.deployment.resources[]` to inject their values into the Helm chart values used to deploy the extension (see [Referencing Resources in Helm Values](/docs/gardener/extensions/registration/#referencing-resources-in-helm-values)).
...
### [`Access` Controller](https://github.com/gardener/gardener/tree/master/pkg/operator/controller/virtual/access)
...
### [`Virtual-Cluster-Registrar` Controller](https://github.com/gardener/gardener/tree/master/pkg/operator/controller/virtual/cluster)
...
### [`Gardenlet` Controller](https://github.com/gardener/gardener/tree/master/pkg/operator/controller/gardenlet)
...
For a general overview, see [this document](/docs/gardener/deployment/deploy_gardenlet/).
...
> After the `gardenlet` is running, it uses the [self-upgrade mechanism](/docs/gardener/deployment/deploy_gardenlet_manually/#self-upgrades) by watching the `seedmanagement.gardener.cloud/v1alpha1.Gardenlet` (see [this](/docs/gardener/concepts/gardenlet/#gardenlet-controller) for more details.)
> 
...
> Make sure to apply your `Gardenlet` resources (potentially containing a new version) after the `Garden` resource was successfully reconciled (i.e., after Gardener control plane was successfully rolled out, see [this](/docs/gardener/deployment/version_skew_policy/#supported-component-upgrade-order) for more information.)
...
If the runtime cluster is a seed cluster at the same time, [`gardenlet`'s `Seed` controller](/docs/gardener/concepts/gardenlet/#seed-controller) will not manage the components which were already deployed (and reconciled) by `gardener-operator`.
...
Please refer to [this document](/docs/gardener/shoot-operations/shoot_credentials_rotation/#gardener-provided-credentials) for more details. As of today, `gardener-operator` only creates the following types of credentials (i.e., some sections of the document don't apply for `Garden`s and can be ignored):
...
Read more about it [here](/docs/gardener/extensions/garden-api-access/#renewing-all-garden-access-secrets).
...
Read more about it [here](/docs/gardener/concepts/gardenlet/#rotate-certificates-using-bootstrap-kubeconfig).
...
You have to prepare the environment accordingly by leveraging the [secret's manager capabilities](/contribute/gardener/secrets_management/#migrating-existing-secrets-to-secretsmanager).
...
See also [this document](/docs/gardener/concepts/resource-manager/#tokenrequestor-controller) for more information about the `TokenRequestor`.
...
Again, you have to prepare the environment accordingly by leveraging the [secret's manager capabilities](/contribute/gardener/secrets_management/#migrating-existing-secrets-to-secretsmanager).
...
This is similar to `make gardener-debug` but for Gardener Operator component. Please check [Debugging Gardener](/docs/gardener/deployment/getting_started_locally/#debugging-gardener) for details.
