...
path_base_for_github_subdir:
  from: content/docs/gardener/concepts/etcd.md
  to: etcd.md
title: etcd
prev: false
next: false
...
If `Seed`s specify backups for etcd ([example](https://github.com/gardener/gardener/blob/master/example/50-seed.yaml)), then Gardener and the respective [provider extensions](/docs/gardener/extensions/) are responsible for creating a bucket on the cloud provider's side (modelled through a [BackupBucket resource](/docs/gardener/extensions/resources/backupbucket/)).
The bucket stores backups of `Shoot`s scheduled on that `Seed`.
Furthermore, Gardener creates a [BackupEntry](/docs/gardener/extensions/resources/backupentry/), which subdivides the bucket and thus makes it possible to store backups of multiple shoot clusters.
...
For `Garden`s specifying backups for etcd ([example](https://github.com/gardener/gardener/blob/master/example/operator/20-garden.yaml)), an existing bucket can be provided via the `Garden` specification. If the bucket does not exist yet, it will be created by Gardener and the respective provider extension.
...
The [backup-restore](https://github.com/gardener/etcd-backup-restore) *sidecar* takes care about this job as well.
