...
path_base_for_github_subdir:
  from: content/docs/gardener/upgrade-gardener.md
  to: upgrade-gardener.md
persona: Operators
title: Upgrade Gardener
prev: false
next: false
---
<!-- BANNER:MANAGED -->
<!--
   ...
   └───────────│────────────────────────────────────┘
               ▼               
   https://github.com/gardener/gardener/blob/master/docs/operations/upgrade-gardener.md
-->
...
First, understand Gardener's [version skew policy](/docs/gardener/deployment/version_skew_policy/), which defines which component versions are compatible with each other.
...
Gardener introduces breaking changes cautiously to ensure stability. You can read the full policy [here](/contribute/gardener/process/#deprecations-and-backwards-compatibility). The release notes will always highlight when you need to perform manual steps.
...
> Hence, they only become relevant when they upgrade Kubernetes - see also [this document](/docs/gardener/shoot/shoot_kubernetes_versions/).
...
After updating your configuration files, you can deploy the new `gardener-operator` using its [Helm chart](https://github.com/gardener/gardener/tree/master/charts/gardener/operator). Once the new operator is running, it will automatically begin updating the `Garden` resource. This process rolls out the new versions of the Gardener control plane components, such as `gardener-apiserver` and `gardener-controller-manager`.

### Image Vector and Overwrites

Gardener components and extensions use an "image vector" to define the specific container images they deploy. If your organization requires using a private container registry, you can replicate the official images and configure Gardener to use them. Follow the instructions [here](/docs/gardener/deployment/image_vector/) to create an image vector overwrite.
...
To prevent overloading the system, these upgrades are staggered using a "jitter" period, so they won't all start at once. By default, this period is 5 minutes. You can adjust it in the `gardenlet`'s [component configuration](https://github.com/gardener/gardener/blob/master/example/20-componentconfig-gardenlet.yaml) by setting `.controllers.managedSeed.syncJitterPeriod`. Set it to `0` to start all upgrades immediately, or increase it if you have many seed clusters to manage the load.
...
To enable this, set `.controllers.shoot.reconcileInMaintenanceOnly=true` in the `gardenlet`'s [component configuration](https://github.com/gardener/gardener/blob/master/example/20-componentconfig-gardenlet.yaml). When this setting is enabled, all `Shoot`s will be reconciled during their next scheduled maintenance window, which typically occurs within 24 hours. You can learn more about shoot maintenance [here](/docs/gardener/shoot/shoot_maintenance/#cluster-reconciliation).
