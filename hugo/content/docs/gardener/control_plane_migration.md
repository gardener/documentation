...
path_base_for_github_subdir:
  from: content/docs/gardener/control_plane_migration.md
  to: control_plane_migration.md
persona: Operators
title: Control Plane Migration
prev: false
next: false
---
<!-- BANNER:MANAGED -->
<!--
   ...
   └───────────│────────────────────────────────────┘
               ▼               
   https://github.com/gardener/gardener/blob/master/docs/operations/control_plane_migration.md
-->
...
1. Kubernetes API Server is stopped and the extension resources are annotated with `gardener.cloud/operation=migrate`.
1. Full snapshot of the ETCD is created and terminating of the Control Plane in the `Source Seed` is initiated.
...
For control plane migration, operators with the necessary RBAC can use the [`shoots/binding`](/docs/gardener/concepts/scheduler/#shootsbinding-subresource) subresource to change the `.spec.seedName`, with the following commands:
