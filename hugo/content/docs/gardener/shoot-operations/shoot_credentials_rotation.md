---
aliases:
  - /docs/gardener/shoot_credentials_rotation/
github_repo: 'https://github.com/gardener/gardener'
github_subdir: docs/usage/shoot-operations
params:
  github_branch: master
path_base_for_github_subdir:
  from: content/docs/gardener/shoot-operations/shoot_credentials_rotation.md
  to: shoot_credentials_rotation.md
persona: Users
title: Shoot Credentials Rotation
prev: false
next: false
---
<!-- BANNER:MANAGED -->
<!--
   ...
   └───────────│────────────────────────────────────┘
               ▼               
   https://github.com/gardener/gardener/blob/master/docs/usage/shoot-operations/shoot_credentials_rotation.md
-->
...
| --- | --- | --- | --- |
| **Infrastructure credentials** | `Project`-scoped — a single `Secret` can be shared across multiple `Shoot`s | Fully user-managed | Update the `Secret`, wait for `Shoot` reconciliation |
| **Shoot credentials** | Per-`Shoot` — generated by Gardener and bound to the `Shoot`'s lifecycle | Managed by Gardener, user-triggered rotation | Multi-phase, annotation-driven operations on the `Shoot` |

Infrastructure credentials are **not** part of the `Shoot` itself. They are `Secret`s stored in the garden cluster's `Project` namespace and referenced by `Shoot`s via a `CredentialsBinding`.
In contrast, `Shoot` credentials (CAs, SSH keys, ETCD encryption keys, etc.) are generated per `Shoot` by Gardener and rotate through well-defined phases driven by annotating operations on the `Shoot` resource.

## Infrastructure Credentials (Project-Scoped)

> This section covers rotation of cloud provider credentials you supplied to Gardener — for example, an Azure Service Principal, AWS access keys, a GCP service account key, or OpenStack credentials. These are project-level `Secret`s referenced by `Shoot`s via `CredentialsBinding`; they are not part of the `Shoot` itself and may be shared across multiple `Shoot`s.
> For rotating credentials that Gardener generates and manages per `Shoot` (CAs, SSH key pair, ETCD encryption key, etc.), see [Shoot Credentials (Gardener-Managed)](#shoot-credentials-gardener-managed).
...
> [!WARNING]
> The legacy `SecretBinding` (`spec.secretBindingName`) is deprecated and cannot be used for Shoots running Kubernetes v1.34 or later — migrate to `CredentialsBinding` if you haven't already (see [SecretBinding to CredentialsBinding Migration](/docs/gardener/shoot-operations/secretbinding-to-credentialsbinding-migration/)).
...
- [AWS](/docs/extensions/infrastructure-extensions/gardener-extension-provider-aws/usage/#provider-secret-data)
- [Azure](/docs/extensions/infrastructure-extensions/gardener-extension-provider-azure/usage/#provider-secret-data)
- [GCP](/docs/extensions/infrastructure-extensions/gardener-extension-provider-gcp/usage/#provider-secret-data)
- [OpenStack](/docs/extensions/infrastructure-extensions/gardener-extension-provider-openstack/usage/#provider-secret-data)
- [Canonical example `Secret` YAML](https://github.com/gardener/gardener/blob/master/example/70-secret-provider.yaml)
...
1. ⚠️ Wait until all `Shoot`s using the `Secret` are reconciled before you disable the old credentials in your cloud provider account! Otherwise, the `Shoot`s will no longer work as expected.
   Each `Shoot` reconciles automatically at least once every 24 hours during its maintenance window. To trigger [an immediate reconciliation](/docs/gardener/shoot-operations/shoot_operations/#immediate-reconciliation):
   ```bash
   kubectl -n garden-<project-name> annotate shoot <shoot-name> gardener.cloud/operation=reconcile
   ```
   - (Optional) To verify the new credentials are valid, trigger an [immediate maintenance operation](/docs/gardener/shoot-operations/shoot_operations/#immediate-maintenance) — this exercises infrastructure reconciliation using the new credentials:
   ```bash
...
> This section covers rotation of credentials that Gardener generates and manages per `Shoot` — CAs, SSH key pair, ETCD encryption key, `ServiceAccount` signing key, and observability passwords. These credentials are cluster-scoped, generated at Shoot creation time, and belong to the Shoot's lifecycle.
> For rotating project-scoped cloud provider credentials, see [Infrastructure Credentials (Project-Scoped)](#infrastructure-credentials-project-scoped).
...
| --- | --- |
| Certificate Authorities | No — fully manual, multi-phase |
| `ServiceAccount` Token Signing Key | No — fully manual, multi-phase |
| SSH Key Pair | Yes — via `.spec.maintenance.autoRotation.credentials.sshKeypair` |
| ETCD Encryption Key | Yes — via `.spec.maintenance.autoRotation.credentials.etcdEncryptionKey` (enabled by default on new `Shoot`s) |
...
See [Automatic Credentials Rotation](/docs/gardener/shoot/shoot_maintenance/#automatic-credentials-rotation) for configuration details.
...
Refer to [Automatic Credentials Rotation](/docs/gardener/shoot/shoot_maintenance/#automatic-credentials-rotation) for instructions on enabling automatic rotation for observability passwords.
...
**Unless automatic credentials rotation is enabled, it is the responsibility of the end-user to regularly rotate those credentials.**
Refer to [Automatic Credentials Rotation](/docs/gardener/shoot/shoot_maintenance/#automatic-credentials-rotation) for instructions on enabling automatic rotation for ssh keypair.
...
This key is used to encrypt the data of `Secret` resources (and any additional resources configured in `spec.kubernetes.kubeAPIServer.encryptionConfig` — see [ETCD Encryption Config](/docs/gardener/security/etcd_encryption_config/)) inside etcd (see [upstream Kubernetes documentation](https://kubernetes.io/docs/tasks/administer-cluster/encrypt-data/)).

The encryption key has no expiration date.
**Unless automatic credentials rotation is enabled, it is the responsibility of the end-user to regularly rotate those credentials.**
Refer to [Automatic Credentials Rotation](/docs/gardener/shoot/shoot_maintenance/#automatic-credentials-rotation) for instructions on enabling automatic rotation for etcd encryption key.
...
- In stage two, all `Secret`s in the cluster and resources configured in the `spec.kubernetes.kubeAPIServer.encryptionConfig` of the Shoot (see [ETCD Encryption Config](/docs/gardener/security/etcd_encryption_config/)) are rewritten by the `kube-apiserver` so that they become encrypted with the new encryption key.
