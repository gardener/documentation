...
Please consult the documentation of the respective provider extension documentation to get to know the concrete data keys (e.g., [this document for AWS](/docs/extensions/infrastructure-extensions/gardener-extension-provider-aws/usage/#provider-secret-data)).
...
1. ⚠️ Wait until all `Shoot`s using the `Secret` are reconciled before you disable the old credentials in your cloud provider account! Otherwise, the `Shoot`s will no longer work as expected. Check out [this document](/docs/gardener/shoot-operations/shoot_operations/#immediate-reconciliation) to learn how to trigger a reconciliation of your `Shoot`s.
   - (Optional) If you want to verify that the new credentials are valid you can trigger [immediate maintenance operation](/docs/gardener/shoot-operations/shoot_operations/#immediate-maintenance) for the corresponding shoot cluster(s) and wait for to subsequent reconciliation(s) to complete successfully. The maintenance operation triggers infrastructure reconciliation during the subsequent shoot reconciliation which makes use of the new cloud provider credentials.
1. After all `Shoot`s using the `Secret` were reconciled, you can go ahead and deactivate the old credentials in your provider account.
