...
The `gardener-operator` runs a [`NetworkPolicy` controller](/docs/gardener/concepts/operator/#networkpolicy-controller-registrar) which is responsible for the following namespaces:
...
Apart from those, the `gardener-operator` also enables the [`NetworkPolicy` controller of `gardener-resource-manager`](/docs/gardener/concepts/resource-manager/#networkpolicy-controller).
...
See also [this document](/contribute/gardener/monitoring-stack/) for more information.
Each pod that should be scraped for metrics by these instances must have a `Service` which is annotated with
