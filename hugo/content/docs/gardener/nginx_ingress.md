---
description: >-
  Guide for landscape operators and shoot cluster owners on how to deal with the
...
path_base_for_github_subdir:
  from: content/docs/gardener/nginx_ingress.md
  to: nginx_ingress.md
persona: Operators
...
Set the feature gate in the [`gardener-operator` component configuration](https://github.com/gardener/gardener/blob/master/example/operator/10-componentconfig.yaml):
...
In the [`Gardenlet` resource](https://github.com/gardener/gardener/blob/master/example/55-gardenlet.yaml) or the [gardenlet component configuration](https://github.com/gardener/gardener/blob/master/example/20-componentconfig-gardenlet.yaml):
...
In the respective component configurations in the [`Garden` resource](https://github.com/gardener/gardener/blob/master/example/operator/20-garden.yaml):
...
1. **Check installed extensions** in the Garden cluster for any that create `Ingress` resources. Known extensions that may use `Ingress` resources include:
...
1. **Check for annotations** referencing the nginx ingress class:
...
1. After confirming no active Ingresses use `nginx-ingress-gardener`, enable `DisableNginxIngressInGarden` for the gardener-operator.
...
1. After confirming no active Ingresses use `nginx-ingress-gardener`, enable `DisableNginxIngressInSeed` for the gardenlet on that Seed.
...
1. **Communicate with shoot owners** that they need to migrate their ingress workloads before you enforce the disablement. See the [Shoot Cluster Owner Guide](#shoot-cluster-owner-guide) for migration instructions.

1. **Set the feature gate in `gardener-apiserver`** first. This prevents new shoots from re-enabling the addon and blocks enabling it on existing shoots.

1. **Set the feature gate in `gardener-controller-manager`** at a potentially later point in time for triggering the removal of nginx-ingress from all shoot clusters. This will disable the addon during the next maintenance window for all shoots that still have it enabled. The change appears in `.status.lastMaintenance` of each affected shoot.
...
1. **Migrate extensions**: Ensure all extensions that create `Ingress` resources have been updated to use an alternative (e.g., Istio `VirtualService`, Traefik `IngressRoute`, or Gateway API resources).
1. **Communicate with shoot owners**: Give shoot owners time to migrate their workloads (see [Shoot Cluster Owner Guide](#shoot-cluster-owner-guide)).
1. **Disable in Garden runtime**: Enable `DisableNginxIngressInGarden` for `gardener-operator` after verifying no Garden-level `Ingress` resources remain.
1. **Disable in Seeds**: Enable `DisableNginxIngressInSeed` for the gardenlet after verifying no Seed-level `Ingress` resources remain.
1. **Disable in Shoots for new cluster**: Enable `DisableNginxIngressInShoot` for `gardener-apiserver`.
1. **Disable in Shoots for all clusters**: Enable `DisableNginxIngressInShoot` for `gardener-controller-manager`.
...
1. Creates a `LoadBalancer` Service that exposes ports 80 (HTTP) and 443 (HTTPS).
1. Creates a wildcard DNS record `*.ingress.<shoot-domain>` pointing to the load balancer's IP or hostname.
...
1. Migrate `Ingress` resources one by one or in batches by changing their ingress class.
1. Verify each migrated service is working correctly with Traefik.
1. Disable the nginx-ingress addon once all `Ingress` resources are migrated.
