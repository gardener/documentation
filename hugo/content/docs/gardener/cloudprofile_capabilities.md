...
path_base_for_github_subdir:
  from: content/docs/gardener/cloudprofile_capabilities.md
  to: cloudprofile_capabilities.md
persona: Operators
title: Cloudprofile Capabilities
prev: false
next: false
---
<!-- BANNER:MANAGED -->
<!--
   ...
   └───────────│────────────────────────────────────┘
               ▼               
   https://github.com/gardener/gardener/blob/master/docs/operations/cloudprofile_capabilities.md
-->


# `CloudProfile` Capabilities

This document describes the capability mechanism in `spec.machineCapabilities` of `CloudProfile` resources. Capabilities let operators express the compatibility matrix between **machine types** and **machine image versions** so that Gardener can:
...
- [Example: Complete `CloudProfile`](#example-complete-cloudprofile)
...
Capabilities replace those workarounds with a structured compatibility contract that lives in the `CloudProfile` and is understood by Gardener and provider extensions.

## Activation

Capabilities are gated by the `CloudProfileCapabilities` feature gate on `gardener-apiserver`. Refer to the [feature gates documentation](/docs/gardener/deployment/feature_gates/) for the current promotion state and the Gardener versions in which it is enabled by default. While the gate is disabled, `spec.machineCapabilities` is rejected on `CloudProfile` admission.

> [!NOTE]
> Capabilities are opt-in: The mechanism does not require migrating all `CloudProfiles` at once. Those without `spec.machineCapabilities` continue using the legacy format (dedicated `architecture` fields) and are validated against legacy rules. There is no automatic conversion.
...
```yaml
spec:
  # 1. Definition: defines all allowed capability names and values.
  machineCapabilities: [...]
...
1. **Value order** (most to least preferred): Within each capability, the first value is the most-preferred. For example, `storageAccess: [NVMe, SCSI]` means "prefer NVMe; use SCSI only if necessary."

Practical guidance:

- **List values from best to legacy.** Put modern/performant options first. Example: `storageAccess: [NVMe, SCSI]` steers maintenance upgrades toward `NVMe` (the more modern storage interface) whenever both are compatible.
...
See also: [Shoot Maintenance — Automatic Version Updates](/docs/gardener/shoot/shoot_maintenance/#automatic-version-updates).
...
Each provider extension is responsible for validating that its API is consistent with the general `CloudProfile` API — one entry in the provider section per `spec.machineImages[].versions[].capabilityFlavors` entry in the general section. In practice, most provider extensions populate this section from their own data, so the contract is typically maintained by construction.
...
A combination is **compatible** if every capability has a non-empty intersection. Otherwise the combination is rejected.
...
1. For each capability, compare the most-preferred supported value of each candidate (using the value order in `spec.machineCapabilities`).
1. The first capability where the candidates differ decides the winner.
...
- **Shoot validator admission** — rejects user-selected machine type / image combinations that are incompatible.
  See [`ShootValidator` admission plugin](/docs/gardener/concepts/apiserver-admission-plugins/#shootvalidator); implemented in [`plugin/pkg/shoot/validator/admission.go`](https://github.com/gardener/gardener/blob/master/plugin/pkg/shoot/validator/admission.go) (function `validateMachineCapabilities`).
- **Shoot mutator admission** — selects a default image when the user does not specify one.
  See [`ShootMutator` admission plugin](/docs/gardener/concepts/apiserver-admission-plugins/#shootmutator); implemented in [`plugin/pkg/shoot/mutator/admission.go`](https://github.com/gardener/gardener/blob/master/plugin/pkg/shoot/mutator/admission.go).
- **Maintenance controller** — picks the upgrade target during automatic image maintenance.
  See [Shoot Maintenance — Automatic Version Updates](/docs/gardener/shoot/shoot_maintenance/#automatic-version-updates); implemented in [`pkg/controllermanager/controller/shoot/maintenance/helper/helper.go`](https://github.com/gardener/gardener/blob/master/pkg/controllermanager/controller/shoot/maintenance/helper/helper.go) (function `FilterMachineImageVersions`).
- **Provider extension worker controller** — selects the actual provider image reference for a given flavor.
  Lives in each provider-extension repo (e.g. `gardener-extension-provider-aws`); the reusable helper for extensions is [`extensions/pkg/controller/worker/cloudprofile.go`](https://github.com/gardener/gardener/blob/master/extensions/pkg/controller/worker/cloudprofile.go) (`FindBestImageFlavor`). An in-tree reference implementation is the local provider's [`pkg/provider-local/controller/worker/machine_images.go`](https://github.com/gardener/gardener/blob/master/pkg/provider-local/controller/worker/machine_images.go).

### Example: Tie-Breaking on `storageAccess`

Consider this minimal `CloudProfile` fragment with a single machine type that supports **both** storage controllers and two image flavors that each support **only one**:
...
spec:
  ...
  - name: architecture
    values: [amd64]
  - name: storageAccess        # most-preferred first
    values: [NVMe, SCSI]
  ...
  - name: general-medium
    capabilities:
      architecture:  [amd64]
      storageAccess: [NVMe, SCSI]   # supports both
  ...
  - name: ubuntu
    ...
    - version: 1.0.0
      capabilityFlavors:
      - architecture:  [amd64]      # Flavor A: NVMe only
        storageAccess: [NVMe]
      - architecture:  [amd64]      # Flavor B: SCSI only
        storageAccess: [SCSI]
```

**Step 1 — Compatibility check.** Intersect the machine type's value set with each flavor's value set, capability by capability:

| Capability | Machine type | Flavor A `[NVMe]` | Flavor B `[SCSI]` |
| --- | --- | --- | --- |
| `architecture` | `[amd64]` | `[amd64]` ✅ | `[amd64]` ✅ |
| `storageAccess` | `[NVMe, SCSI]` | `[NVMe]` ✅ | `[SCSI]` ✅ |

Both flavors are compatible — neither intersection is empty.

**Step 2 — Tie-breaking.** Walk capabilities in the order they are registered in `spec.machineCapabilities`:

1. `architecture`: both candidates' most-preferred supported value is `amd64`. Tie, move on.
1. `storageAccess`: Flavor A's most-preferred supported value is `NVMe`; Flavor B's is `SCSI`. The value order in `spec.machineCapabilities` is `[NVMe, SCSI]` → `NVMe` comes first → **Flavor A wins**.
...
- **Never declare `spec.machineCapabilities`.** The definition is inherited from the parent `CloudProfile`.
...
### Example: Inheriting and Overriding Versions
...
The capability mechanism is intentionally small but rigid. A handful of design rules will keep `CloudProfile`s maintainable:
...
1. **Order from best to legacy.** This ensures that maintenance auto-upgrades steer fleets toward the best possible image when multiple flavors are valid. Example: `storageAccess: [NVMe, SCSI]` prefers `NVMe`.
1. **Design for stability within the `CloudProfile`.** A capability should capture a meaningful axis of compatibility that won't change frequently. It's fine to add new values to an existing capability or rename it within a `CloudProfile` — just update all declarations consistently. The risk is `Shoot`s or `NamespacedCloudProfile`s that depend on the capability; removing or drastically redefining capabilities they reference requires careful coordination.
1. **Watch the size budget.** A `CloudProfile` is an etcd object with the standard 1.5 MiB limit. Capability flavors multiply per image version; a profile with many images and many flavors can grow quickly.

## Migration from Legacy Fields

The capability mechanism is purely additive. A staged migration looks like this:

1. **Gardener core** is upgraded to a version with the `CloudProfileCapabilities` feature gate Beta or GA. Nothing changes for existing `CloudProfile`s.
1. **Provider extensions** are upgraded to a version that understands `capabilityFlavors` in their `providerConfig` section. `CloudProfile`s still work without capabilities.
1. **Per `CloudProfile`**, the operator opts in by adding `spec.machineCapabilities` and migrating each machine type and each image version. The legacy `architecture` fields do not need to be removed: Gardener keeps them in sync with the capability values automatically (see [The `architecture` Capability](#the-architecture-capability)).

There is no requirement to migrate all `CloudProfile`s at the same time. Profiles that have not opted in continue to be validated against the legacy rules.

## Example: Complete `CloudProfile`
