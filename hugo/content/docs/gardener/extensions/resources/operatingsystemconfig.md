...
github_subdir: docs/extensions/resources
params:
  github_branch: master
path_base_for_github_subdir:
  from: content/docs/gardener/extensions/resources/operatingsystemconfig.md
  to: operatingsystemconfig.md
...
<!--
   ...
   └───────────│────────────────────────────────────┘
               ▼               
   https://github.com/gardener/gardener/blob/master/docs/extensions/resources/operatingsystemconfig.md
-->
...
You can find the complete configuration [at the components folder](https://github.com/gardener/gardener/tree/master/pkg/component/extensions/operatingsystemconfig/original/components). We are calling this the "original" user-data.
...
Please refer to [this document](/docs/gardener/concepts/node-agent/#installation-and-bootstrapping) for a first overview.
...
Instead, we only send a small "init" script that bootstrap the [`gardener-node-agent`](/docs/gardener/concepts/node-agent/).
...
As part of the [`Shoot` reconciliation flow](/docs/gardener/concepts/gardenlet/#shoot-controller), `gardenlet` will create a special CRD in the seed cluster that needs to be reconciled by an extension controller, for example:
...
You can find an example implementation [here](https://github.com/gardener/gardener/blob/master/pkg/provider-local/controller/operatingsystemconfig/actuator.go).
...
The token is used to bootstrap [Gardener Node Agent](/docs/gardener/concepts/node-agent/) and `kubelet`.
...
However, there are cases, most notably the control plane worker pool of [self-hosted shoot clusters](https://github.com/gardener/gardener/blob/master/docs/proposals/28-self-hosted-shoot-clusters.md), where you need to apply slightly different configurations to different nodes of the same worker pool.
...
For a convenient handling, [gardener-node-agent](/docs/gardener/concepts/node-agent/) can manage various aspects of containerd's config, e.g. the registry configuration, if given in the `OperatingSystemConfig`.
