...
path_base_for_github_subdir:
  from: content/docs/gardener/extensions/resources/containerruntime.md
  to: containerruntime.md
...
<!--
   ...
   └───────────│────────────────────────────────────┘
               ▼               
   https://github.com/gardener/gardener/blob/master/docs/extensions/resources/containerruntime.md
-->
...
Gardener supports creation of Worker machines using CRI. For more information, see [CRI Support](/docs/gardener/extensions/resources/operatingsystemconfig/#cri-support).
...
Additionally, Gardener labels `OperatingSystemConfig` resources generated for worker pools as well as the Shoot control plane namespace on the Seed cluster with the same container runtime labels (`containerruntime.worker.gardener.cloud/<container-runtime-type-value>=true`) so that extension mutating webhooks can target only relevant objects or namespaces using an `ObjectSelector` or `NamespaceSelector`.
Note that this label prefix (`containerruntime.worker.gardener.cloud/`) differs from the `extensions.gardener.cloud/<type>` labels used by other extension types (and produced by `extensionswebhook.BuildExtensionTypeNamespaceSelector`). Container runtime extension webhooks targeting `OperatingSystemConfig`s or the shoot control plane namespace can use `extensionswebhook.BuildContainerRuntimeTypeObjectSelector` or `extensionswebhook.BuildContainerRuntimeTypeNamespaceSelector` (or construct a selector for `containerruntime.worker.gardener.cloud/<type>=true` directly).
