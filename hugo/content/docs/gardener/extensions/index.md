...
path_base_for_github_subdir:
  from: content/docs/gardener/extensions/_index.md
  to: overview.md
title: Extensions
weight: 30
prev: false
...
<!--
   ...
   └───────────│────────────────────────────────────┘
               ▼               
   https://github.com/gardener/gardener/blob/master/docs/extensions/overview.md
-->
...
1. Optionally, a Helm chart with an admission component inside. The admission controller runs in the garden runtime cluster and validates extension specific settings of the `Shoot` (given in `providerConfig` fields). See [admission](/docs/gardener/extensions/admission/) for more details.

### Registration

Before an extension can be used, it needs to be made known to the system. The [gardener-operator](/docs/gardener/concepts/operator/) automates much of the registration process, making `Extension` resources (group `operator.gardener.cloud`) the preferred method for registering extensions. For more information, see the [Registration documentation](/docs/gardener/extensions/registration/).
...
Extensions are defined by their Kinds (defined by Gardener - see [resources](https://github.com/gardener/gardener/tree/master/docs/extensions/resources)) and Types.
...
We track all extensions of Gardener in the [known Gardener Extensions List](/docs/extensions/#known-extension-implementations) repo.
