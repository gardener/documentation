...
path_base_for_github_subdir:
  from: content/docs/gardener/extensions/controlplane-webhooks.md
  to: controlplane-webhooks.md
...
<!--
   ...
   └───────────│────────────────────────────────────┘
               ▼               
   https://github.com/gardener/gardener/blob/master/docs/extensions/controlplane-webhooks.md
-->
...
* Initiates the deployment of custom controlplane components by [ControlPlane controllers](/docs/gardener/extensions/resources/controlplane/) by creating a `ControlPlane` resource in the Shoot namespace.
...
The controlplane of a virtual garden managed by the [gardener-operator](/docs/gardener/concepts/operator/) contains similar *core components* as a regular workerless shoot cluster.
Extensions controllers deploying webhooks for the garden controlplane may follow the same principles.
In this scope, the term *Shoot* refers to the virtual garden, whereas the term *Seed* refers to the physical/runtime cluster hosting the virtual garden.
...
> **Note:** The contract described below may not necessarily be what Gardener does currently (as of May 2019). Rather, it reflects the target state after changes for [Gardener extensibility](/docs/gardener/extensions/) have been introduced.
...
Gardener **shall not** deploy a cloud-controller-manager. If it is needed, it should be added by a [`ControlPlane` controller](/docs/gardener/extensions/resources/controlplane/)

### CSI Controllers

Gardener **shall not** deploy a CSI controller. If it is needed, it should be added by a [`ControlPlane` controller](/docs/gardener/extensions/resources/controlplane/)

### kubelet

To specify the kubelet configuration, Gardener **shall** create a [`OperatingSystemConfig` resource](/docs/gardener/extensions/resources/operatingsystemconfig/) with any name and purpose `reconcile` in the Shoot namespace. It can therefore also be mutated by webhooks to apply any provider-specific changes to the standard configuration provided by Gardener. Gardener **may** write multiple such resources with different `type` to the same Shoot namespaces if multiple OSs are used.
