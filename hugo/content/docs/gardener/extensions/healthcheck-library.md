...
path_base_for_github_subdir:
  from: content/docs/gardener/extensions/healthcheck-library.md
  to: healthcheck-library.md
...
<!--
   ...
   └───────────│────────────────────────────────────┘
               ▼               
   https://github.com/gardener/gardener/blob/master/docs/extensions/healthcheck-library.md
-->
...
For more information, see [Contributing to Shoot Health Status Conditions](/docs/gardener/extensions/shoot-health-status-conditions/).

A health check has to [satisfy the below interface](https://github.com/gardener/gardener/blob/master/extensions/pkg/controller/healthcheck/actuator.go).
You can find implementation examples in the [healtcheck folder](https://github.com/gardener/gardener/tree/master/extensions/pkg/controller/healthcheck/general).
...
The [Gardener Resource Manager](/docs/gardener/concepts/resource-manager/) conducts basic checks for different API objects out-of-the-box (e.g `Deployments`, `DaemonSets`, ...) for objects deployed via `ManagedResource`s (see [example 1](https://github.com/gardener/gardener/blob/e5bd1127959f5756bfcaf0884bf00a0b2e8bd344/pkg/component/observability/opentelemetry/collector/collector.go#L157-L159), [example 2](https://github.com/gardener/gardener/blob/a7029002ef6e68b9b37df11fea934bcc80ce6f2c/pkg/component/observability/logging/fluentoperator/fluentoperator.go#L300)).
...
These checks are performed by the Gardenlet and maintained in the [Shoot Care Reconciler](/docs/gardener/concepts/gardenlet/#care-reconciler-2).

### Seed Managed Resources

For the `Seed` object, health checks for `ManagedResource`s are performed by the [Seed Care Reconciler](/docs/gardener/concepts/gardenlet/#care-reconciler-1).
...
For the `Garden` object managed by the Gardener Operator, health checks for `ManagedResource`s are performed by the [Garden Care Reconciler](/docs/gardener/concepts/operator/#care-reconciler).
