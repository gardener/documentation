...
path_base_for_github_subdir:
  from: content/docs/gardener/extensions/garden-api-access.md
  to: garden-api-access.md
...
<!--
   ...
   └───────────│────────────────────────────────────┘
               ▼               
   https://github.com/gardener/gardener/blob/master/docs/extensions/garden-api-access.md
-->
...
For this, `gardenlet` runs an instance of the `TokenRequestor` for [`ServiceAccount`s](/docs/gardener/concepts/gardenlet/#tokenrequestor-controller-for-serviceaccounts) or [`WorkloadIdentity`s](/docs/gardener/concepts/gardenlet/#tokenrequestor-controller-for-workloadidentitys) for requesting tokens that can be used to communicate with the garden cluster.
...
When installing a [`ControllerInstallation`](/docs/gardener/extensions/registration/#deploying-extension-controllers), gardenlet creates two secrets in the installation's namespace: a generic garden kubeconfig (`generic-garden-kubeconfig-<hash>`) and a garden access secret (`garden-access-extension`).
...
Both the [`SeedAuthorizer` and the `SeedRestriction` plugin](/docs/gardener/deployment/gardenlet_api_access/) handle extensions clients and generally grant the same permissions in the garden cluster to them as to gardenlet clients.
...
There are a few exceptions to the granted permissions as documented [here](/docs/gardener/deployment/gardenlet_api_access/#rule-exceptions-for-extension-clients).
...
You can read more about this controller [here](/docs/gardener/concepts/controller-manager/#extension-clusterrole-reconciler).
