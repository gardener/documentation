...
path_base_for_github_subdir:
  from: content/docs/gardener/extensions/static-manifests.md
  to: static-manifests.md
title: Static Manifests
prev: false
next: false
---
<!-- BANNER:MANAGED -->
<!--
   ...
   └───────────│────────────────────────────────────┘
               ▼               
   https://github.com/gardener/gardener/blob/master/docs/extensions/static-manifests.md
-->
...
1. Copies all matching `Secret`s into each Shoot namespace.
1. Creates a single `ManagedResource` that references these `Secret`s.
1. The `ManagedResource` ensures the manifests are applied to the Shoot cluster.
...
1. **Check the `ManagedResource`** referencing your `Secret`:
   ```bash
   kubectl get managedresource static-manifests-from-seed -n shoot--<project>--<shoot>
   ```

1. **Check the Shoot cluster** to confirm resources are applied:
   ```bash
...
1. Wait for the next Shoot reconciliation cycle, or manually trigger reconciliation.
1. The `gardenlet` will detect the change and update the `ManagedResource` in each Shoot namespace.
1. The updated manifests will be applied to all Shoot clusters.
...
1. During the next reconciliation, the `gardenlet` will remove the `Secret` from Shoot namespaces.
1. The associated resources will be deleted from the Shoot clusters via the `ManagedResource` cleanup.
