...
path_base_for_github_subdir:
  from: content/docs/gardener/extensions/conventions.md
  to: conventions.md
title: Conventions
prev: false
next: false
...
<!--
   ...
   └───────────│────────────────────────────────────┘
               ▼               
   https://github.com/gardener/gardener/blob/master/docs/extensions/conventions.md
-->


# General Conventions

All the extensions that are registered to Gardener are deployed to the garden runtime and seed clusters on which they are required (also see [extension registration documentation](/docs/gardener/extensions/registration/)).
...
Instead, they can and should rely on well-known [`PriorityClasses`](/contribute/gardener/priority-classes/) managed by gardenlet.
...
In case a seed or shoot cluster is highly available, there are various failure tolerance types. For more information, see [Highly Available Shoot Control Plane](/docs/guides/high-availability/control-plane/).
...
Please refer to [High Availability of Deployed Components](/contribute/gardener/high-availability-of-components/) for details.
...
Please refer to [this document](/docs/gardener/topology_aware_routing/) for details.
...
Extensions are supposed to use [`ManagedResources`](/docs/gardener/concepts/resource-manager/#ManagedResource-controller) to manage resources in shoot clusters.
...
If you need to deploy a non-DaemonSet resource, Gardener automatically ensures that it only runs on nodes that are allowed to host system components and extensions. For more information, see [System Components Webhook](/docs/gardener/concepts/resource-manager/#System-Components-Webhook).
...
Extensions need to look up the `generic-token-kubeconfig.secret.gardener.cloud/name` annotation on the respective [`Cluster`](/docs/gardener/extensions/cluster/) object in order to determine which secret contains the current CA bundle.
The helper function `extensionscontroller.GenericTokenKubeconfigSecretNameFromCluster` can be used for this task.

You can take a look at [CA Rotation in Extensions](/docs/gardener/extensions/ca-rotation/) for more details on the CA rotation feature in regard to extensions.
...
Please see [CA Rotation in Extensions](/docs/gardener/extensions/ca-rotation/) for the exact requirements that extensions need to fulfill in order to support the CA rotation feature.
