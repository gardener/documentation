...
path_base_for_github_subdir:
  from: content/docs/gardener/extensions/logging-and-monitoring.md
  to: logging-and-monitoring.md
...
Gardener provides an integrated logging and monitoring stack for alerting, monitoring, and troubleshooting of its managed components by operators or end users. For further information how to make use of it in these roles, refer to the corresponding guides for [exploring logs](https://github.com/gardener/logging/blob/master/docs/usage.md) and for [monitoring with Plutono](https://github.com/credativ/plutono).
...
For more details, continue reading [here](/docs/gardener/monitoring/#prometheus).
...
For more details, continue reading [here](/docs/gardener/monitoring/#seed-prometheus).
...
For more details, continue reading [here](/docs/gardener/monitoring/#aggregate-prometheus).
...
!["Cluster Logging Topology"](/docs/gardener/images/logging-architecture.png "Cluster Logging Topology")
...
- Logs are persisted by Vali instances deployed as StatefulSets - one per shoot namespace, if the logging is enabled in the `gardenlet` configuration (`logging.enabled`) and the [shoot purpose](/docs/gardener/shoot/shoot_purposes/#behavioral-differences) is not `testing`, and one in the `garden` namespace. The shoot instances store logs from the control plane components hosted there. The `garden` Vali instance is responsible for logs from the rest of the seed namespaces - `kube-system`, `garden`, `extension-*`, and others.
...
**Example:** Logging configuration for provider-specific `cloud-controller-manager` deployed into shoot namespaces that reuses the `kube-apiserver-parser` defined in [logging.go](https://github.com/gardener/gardener/blob/master/pkg/component/kubernetes/apiserver/logging.go) to parse the component logs:
...
Further details how to define parsers and use them with examples can be found in the following [guide](/contribute/gardener/log_parsers/).
...
- Garden Plutono dashboards expose logs from non-shoot namespaces of the seed clusters
  - [Pod Logs](https://github.com/gardener/gardener/blob/master/pkg/component/observability/plutono/dashboards/seed/pod-logs.json)
  - [Extensions](https://github.com/gardener/gardener/blob/master/pkg/component/observability/plutono/dashboards/seed/extensions-dashboard.json)
  - [Systemd Logs](https://github.com/gardener/gardener/blob/master/pkg/component/observability/plutono/dashboards/seed/systemd-logs.json)
- Shoot Plutono dashboards expose logs from the shoot cluster namespace where they belong
  ...
  - Cluster Autoscaler
  - VPA components
  - [Kubernetes Pods](https://github.com/gardener/gardener/blob/master/pkg/component/observability/plutono/dashboards/shoot/owners/kubernetes-pods-dashboard.json)
...
- For monitoring to work in the Gardener context, scrape targets need to be labelled appropriately, see [`NetworkPolicy`s In Garden, Seed, Shoot Clusters](/docs/gardener/network_policies/) for details.
...
- [Network Policies for Logging & Monitoring in Garden Runtime Cluster](/docs/gardener/network_policies/#logging--monitoring)
- [Network Policies for Logging & Monitoring in Seed Cluster](/docs/gardener/network_policies/#logging--monitoring-1)
