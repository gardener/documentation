---
aliases:
  - /docs/gardener/shoot-workload-identity/
description: >-
  Configure access to infrastructure accounts via workload identity instead of
  static credentials
...
path_base_for_github_subdir:
  from: content/docs/gardener/shoot/shoot-workload-identity.md
  to: shoot-workload-identity.md
persona: Users
...
<!--
   ...
   └───────────│────────────────────────────────────┘
               ▼               
   https://github.com/gardener/gardener/blob/master/docs/usage/shoot/shoot-workload-identity.md
-->


# Shoot Workload Identity

[`WorkloadIdentity`](/docs/gardener/api-reference/security/#workloadidentity) is a resource that allows workloads to be presented before external systems by giving them identities managed by Gardener.
...
The issuer URL can be read from the [Gardener Info ConfigMap](/docs/gardener/configmap/).

> [!TIP]
> `Shoot`s that were previously using `Secret`s as authentication method can also be migrated to use `WorkloadIdentity`.
> As the `credentialsRef` field of [`CredentialsBinding`](/docs/gardener/api-reference/security/#credentialsbinding) is immutable,
