---
aliases:
  - /docs/gardener/shoot_maintenance/
description: >-
  Defining the maintenance time window, configuring automatic version updates,
  confining reconciliations to only happen during maintenance, adding an
  additional maintenance operation, etc.
github_repo: 'https://github.com/gardener/gardener'
github_subdir: docs/usage/shoot
params:
  github_branch: master
path_base_for_github_subdir:
  from: content/docs/gardener/shoot/shoot_maintenance.md
  to: shoot_maintenance.md
persona: Users
...
<!--
   ...
   └───────────│────────────────────────────────────┘
               ▼               
   https://github.com/gardener/gardener/blob/master/docs/usage/shoot/shoot_maintenance.md
-->
...
Please refer to the [Shoot Kubernetes and Operating System Versioning in Gardener](/docs/gardener/shoot-operations/shoot_versions/) topic for more information about Kubernetes and machine image versions in Gardener.
    ...
    autoRotation:
      credentials:
        ...
        sshKeypair: # set this field to enable automatic rotation
          rotationPeriod: 168h # default: 7d(168h)
        etcdEncryptionKey: # for newly created shoots, this is enabled by default
...
> See [ETCD Encryption Config](/docs/gardener/security/etcd_encryption_config/) for more details.
...
Outside of the maintenance time window it will only reconcile if you change the specification or if you explicitly trigger it, see also [Trigger Shoot Operations](/docs/gardener/shoot-operations/shoot_operations/).
...
⚠️ As exceptions to the above rules, [manually triggered reconciliations](/docs/gardener/shoot-operations/shoot_operations/#immediate-reconciliation) and changes to the `.spec.hibernation.enabled` field trigger immediate rollouts.
...
In case you would like to perform a [shoot credential rotation](/docs/gardener/shoot-operations/shoot_operations/#credentials-rotation-operations) or a `reconcile` operation during your maintenance time window, you can annotate the `Shoot` with
...
A list of operations that can run in parallel can be found in the [Shoot Operations documentation](/docs/gardener/shoot-operations/shoot_operations/#running-operations-in-parallel).

> ⚠️ This is skipped when the `Shoot`'s `.status.lastOperation.state=Failed`. Make sure to [retry](/docs/gardener/shoot-operations/shoot_operations/#retry-failed-reconciliation) your shoot reconciliation beforehand.
...
See [Shoot Maintenance](/docs/gardener/extensions/shoot-maintenance/) to see how extension developers can extend this behaviour.
