...
github_subdir: docs/usage/autoscaling
level: advanced
params:
  github_branch: master
path_base_for_github_subdir:
  from: content/docs/guides/applications/shoot-pod-autoscaling-best-practices.md
  to: shoot_pod_autoscaling_best_practices.md
...
<!--
   ...
   └───────────│────────────────────────────────────┘
               ▼               
   https://github.com/gardener/gardener/blob/master/docs/usage/autoscaling/shoot_pod_autoscaling_best_practices.md
-->
...
In this example, VPA is configured to scale `foo-deployment` requests (`RequestsOnly`) from 50m cores (`minAllowed`) up to 4 cores (`maxAllowed`) and 200M memory (`minAllowed`) up to 16G memory (`maxAllowed`) automatically (`updateMode`). So in `updateMode` `Recreate` it will evict pods under certain conditions and then mutate the requests (and possibly limits if you omit `controlledValues` or set it to `RequestsAndLimits`, which is the default) of upcoming new pods. As of VPA [v1.4.0](https://github.com/kubernetes/autoscaler/releases/tag/vertical-pod-autoscaler-1.4.0), *in-place* Pod resource updates are supported through the `InPlaceOrRecreate` `updateMode` and from Gardener [v1.124.0](https://github.com/gardener/gardener/releases/tag/v1.124.0) can be configured across `Shoot`, `Seed` and `Garden` clusters. Additionally, VPA is configured to ignore all other containers in the pods of the target (`mode: "Off"` for `containerName: '*'`), which is useful if you have sidecar containers that should not be scaled by VPA, e.g. a logging or monitoring sidecar. If you omit this second `containerPolicy`, VPA would apply its DefaultContainerResourcePolicy to all containers which don't have an explicit by-name `containerPolicy`, which is often not what you want.

Multiple [update modes](https://github.com/kubernetes/autoscaler/blob/master/vertical-pod-autoscaler/docs/quickstart.md) exist. They influence eviction and mutation. The available ones are:
...
- `Auto` [**deprecated**]: VPA assigns resource requests on pod creation as well as updates them on existing pods using the preferred update mechanism. Currently, this is equivalent to `Recreate`.
...
- `InPlaceOrRecreate`: In this mode, recommendations are computed and applied to pods using the [in-place resource updates](https://kubernetes.io/blog/2025/05/16/kubernetes-v1-33-in-place-pod-resize-beta/) mechanism. In case of failure, it falls back to *recreation* and *evicts* the targeted `Pod`.
- `InPlace`: In this mode, recommendations are computed and applied to pods using the in-place resource updates mechanism. Unlike `InPlaceOrRecreate`, this mode never evicts pods. If an in-place resize cannot be performed, VPA retries the update later when cluster conditions change. This mode is recommended for workloads where any disruption is unacceptable.
