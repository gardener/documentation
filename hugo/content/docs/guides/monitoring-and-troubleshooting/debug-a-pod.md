...
- The examples below are deployed into the namespace `default`. In case you want to change it, use the optional parameter `--namespace <your-namespace>` to select the target namespace. The examples require a Kubernetes release &ge; *1.8*.
...
In case you didn't change the image tag, the default image policy *IfNotPresent* tells Kubernetes to use the cached image (see [Images](https://kubernetes.io/docs/concepts/containers/images/)).
...
For more information, see [Container Image Not Updating](/docs/guides/applications/image-pull-policy/).
