...
1. **Install the operator with Garden Linux configuration:**
...
1. **PrometheusRule**: Create recording rules to aggregate raw DCGM metrics into stable, pod-level custom metrics:
   ```yaml
   apiVersion: monitoring.coreos.com/v1
   kind: PrometheusRule
   metadata:
...
1. **Prometheus Adapter**: Deploy prometheus-adapter to expose Prometheus metrics via the `custom.metrics.k8s.io` API:
   ```bash
...
1. **HPA**: Configure an HPA to use the custom GPU metric for scaling decisions:
   ```yaml
