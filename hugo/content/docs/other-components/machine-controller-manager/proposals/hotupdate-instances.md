...
path_base_for_github_subdir:
  from: >-
    content/docs/other-components/machine-controller-manager/proposals/hotupdate-instances.md
  to: hotupdate-instances.md
title: Hotupdate Instances
prev: false
next: false
---
<!-- BANNER:MANAGED -->
<!--
   ...
   └───────────│────────────────────────────────────┘
               ▼               
   https://github.com/gardener/machine-controller-manager/blob/master/docs/proposals/hotupdate-instances.md
-->
...
1. Additional metadata for struct fields will now be added via `struct tags`.
...
1. Add another function to compute hash using all immutable fields in the provider config struct and then pass that to `worker.WorkerPoolHash` as `additionalData`.
...
1. Enhance the machine reconciliation to include code to hot-update machine.
...
1. `inflight-providerspec-hash` annotation will be removed.
