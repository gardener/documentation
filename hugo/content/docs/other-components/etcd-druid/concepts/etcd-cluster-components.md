...
path_base_for_github_subdir:
  from: content/docs/other-components/etcd-druid/concepts/etcd-cluster-components.md
  to: etcd-cluster-components.md
title: Etcd Cluster Components
prev: false
next: false
managed: true
---
...
`etcd-backup-restore` container running as a side-car in every etcd-member, requires permissions to access resources like `Lease`, `StatefulSet` etc. A dedicated [Role](/docs/other-components/etcd-druid/concepts/etcd-cluster-components/) and [RoleBinding](/docs/other-components/etcd-druid/concepts/etcd-cluster-components/) is created and linked to the [ServiceAccount](https://kubernetes.io/docs/concepts/security/service-accounts/) created per `Etcd` cluster.
