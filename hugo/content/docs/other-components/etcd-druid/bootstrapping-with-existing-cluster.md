...
path_base_for_github_subdir:
  from: >-
    content/docs/other-components/etcd-druid/bootstrapping-with-existing-cluster.md
  to: bootstrapping-with-existing-cluster.md
persona: Users
title: Bootstrapping With Existing Cluster
prev: false
next: false
---
<!-- BANNER:MANAGED -->
<!--
   ...
   └───────────│────────────────────────────────────┘
               ▼               
   https://github.com/gardener/etcd-druid/blob/master/docs/usage/bootstrapping-with-existing-cluster.md
-->
...
For background and lifecycle semantics, see [Bootstrap with an Existing etcd Cluster](/docs/other-components/etcd-druid/concepts/bootstrap-with-existing-cluster/). This guide uses "source cluster", "target `Etcd` resource", and "join phase" as defined in that document's [Terminology](/docs/other-components/etcd-druid/concepts/bootstrap-with-existing-cluster/#terminology) section.
...
1. **The source peer URLs in the target spec are reachable from the target pods.** Only the URLs listed under `.spec.etcd.bootstrapWithExistingCluster.members[*].peerUrls` need to be reachable from the target network.
1. **At least one source client endpoint is reachable from the target backup-restore sidecar.** These endpoints go into `.spec.etcd.bootstrapWithExistingCluster.clientEndpoints`.
...
> 
> For an etcd-druid managed source, configure [`.spec.etcd.additionalAdvertisedURLs`](/docs/other-components/etcd-druid/using-additional-advertised-urls/) on the source if extra reachable peer URLs are needed. For an externally managed source, prepare the advertised peer URLs outside etcd-druid. The target spec must use the source member names and peer URLs as advertised by the source member list.
...
- [Concept doc: Bootstrap with an Existing etcd Cluster](/docs/other-components/etcd-druid/concepts/bootstrap-with-existing-cluster/)
- [Using Additional Advertised URLs](/docs/other-components/etcd-druid/using-additional-advertised-urls/)
- [Securing Etcd Clusters](/docs/other-components/etcd-druid/securing-etcd-clusters/)
