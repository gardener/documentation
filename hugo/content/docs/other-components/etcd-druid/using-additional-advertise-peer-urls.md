...
path_base_for_github_subdir:
  from: >-
    content/docs/other-components/etcd-druid/using-additional-advertise-peer-urls.md
  to: using-additional-advertise-peer-urls.md
persona: Users
title: Using Additional Advertise Peer Urls
prev: false
next: false
---
<!-- BANNER:MANAGED -->
<!--
   ...
   └───────────│────────────────────────────────────┘
               ▼               
   https://github.com/gardener/etcd-druid/blob/master/docs/usage/using-additional-advertise-peer-urls.md
-->
...
1. **DNS or IP routing** configured so that the addresses specified in `additionalAdvertisePeerURLs` are reachable from outside the cluster.
1. **TLS certificates** with SANs covering the external IPs/hostnames, if peer TLS is enabled.
1. Familiarity with [etcd clustering concepts](https://etcd.io/docs/v3.5/op-guide/clustering/).
...
Reconciliation is triggered automatically on `Etcd` resource updates (post `v0.23.0`). Refer to [Managing Etcd Clusters](/docs/other-components/etcd-druid/managing-etcd-clusters/) for further details.
