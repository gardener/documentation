...
path_base_for_github_subdir:
  from: >-
    content/docs/other-components/etcd-druid/proposals/06-immutable-etcd-backups.md
  to: 06-immutable-etcd-backups.md
reviewers:
...
<!--
   ...
   └───────────│────────────────────────────────────┘
               ▼               
   https://github.com/gardener/etcd-druid/blob/master/docs/proposals/06-immutable-etcd-backups.md
-->
...
1. **Bucket-Level Immutability:** Applies a uniform immutability policy to all objects in a bucket.
...
1. **Provide Valid Credentials in a Kubernetes Secret**
...
- See the [Getting Started guide](/docs/other-components/etcd-druid/deployment/getting-started-locally/getting-started-locally/#setting-up-cloud-provider-object-store-secret) for an example.
> [!NOTE]
...
1. **Prerequisite: Cut-off Traffic and Take a Final Full Snapshot Before Hibernation**
   ...
   - The etcd controller then triggers an [on-demand full snapshot](/docs/other-components/etcd-druid/proposals/05-etcdopstask/#trigger-on-demand-fulldelta-snapshot). This ensures that the latest state of the etcd cluster is captured and securely stored before hibernation begins.

1. **Periodically Re-Upload the Snapshot**
...
An alternative solution could be to trigger the [snapshot compaction](/docs/other-components/etcd-druid/proposals/02-snapshot-compaction/) which runs in separate pod and takes a fresh snapshot of the compacted etcd data and re-uploads it to the object store.
...
When a hibernation flow is initiated (by external tooling or higher-level operators), the [etcd controller](/contribute/other-components/etcd-druid/controllers/#etcd-controller) can:

1. Remove etcd’s client ports (2379/2380) from the etcd Service to block application traffic.
1. Trigger an [on-demand full snapshot](/docs/other-components/etcd-druid/proposals/05-etcdopstask/#trigger-on-demand-fulldelta-snapshot) via an `EtcdOpsTask`.
1. Scale down the `StatefulSet` replicas to zero, provided the previous snapshot step is successful.
1. Create the `ExtendFullSnapshotImmutabilityTask` if `etcd.spec.backup.store.immutability.retentionType` is `"bucket"` and based on `etcd.spec.backup.fullSnapshotSchedule`.
...
1. **Use `EtcdCopyBackupsTask`.** If you want to start the cluster with a new bucket but retain old data, use the [`EtcdCopyBackupsTask`](https://github.com/gardener/etcd-druid/blob/master/examples/etcdcopybackstask/druid_v1alpha1_etcdcopybackupstask.yaml) to copy existing backups from the old immutable bucket to the new mutable bucket.
1. **Reconcile the `Etcd` CR.** After pointing `etcd.spec.backup.store` to the new bucket, `etcd-druid` will start storing backups there.
