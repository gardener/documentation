...
![microservices](/docs/getting-started/images/microservices.webp)

Running a highly distributed system has advantages, but of course, they come at a cost. In order to succeed, you need:
...
Take the number of nodes and pods, for example. Both are connected and you cannot grow both towards their individual limits, as you would face issues before reaching any theoretical limits.

Reading the [Scalability of Gardener Managed Kubernetes Clusters](/docs/guides/administer-shoots/scalability/) guide is strongly recommended in order to understand the topic of scalability within Kubernetes and Gardener.

### A Small Sample of Things That Can Grow Beyond Reasonable Limits

![hibernation-1](/docs/getting-started/images/hibernation-1.webp)

When scaling a cluster, there are plenty of resources that can be exhausted or reach a limit:

- The API server will be scaled horizontally and vertically by Gardener. However, it can still consume too many resources to fit onto a single node on the seed. In this case, you can only reduce the load on the API server. This should not happen with regular usage patterns, though.
...
![capacity](/docs/getting-started/images/capacity.webp)
...
![mask-size](/docs/getting-started/images/mask-size.webp)
...
As part of the Kubernetes network setup, the `/16` network is divided into smaller subnets and each node gets a distinct subnet. The size of this subnet defaults to `/24`. It can also be specified via the `nodeCIDRMaskSize` setting (but not changed later).
...
For more information, see [Shoot Networking](/docs/gardener/networking/shoot_networking/).

## Overlapping VPCs

### Avoid Overlapping CIDR Ranges in VPCs

![cidr-ranges](/docs/getting-started/images/cidr-ranges.webp)

Gardener can create shoot cluster resources in an existing VPC. However, you have to make sure that the CIDR ranges used by the shoots nodes or subnets for zones do not overlap with other shoots deployed to the same VPC.

In case of an overlap, there might be strange routing effects, and packets ending up at a wrong location.

## Expired Credentials

![expired-credentials](/docs/getting-started/images/expired-credentials.webp)
...
You can update the credentials stored in the project namespace and reconcile the cluster to replicate the new keys to all relevant controllers. Similarly, when doing a planned rotation, you should wait until the shoot reconciled successfully before invalidating the old credentials.

## AutoUpdate Breaking Clusters

Gardener can automatically update a shoot's Kubernetes patch version, when a new patch version is labeled as "supported". Automatically updating the OS images works in a similar way. Both are triggered by the "supported" classification in the respective cloud profile and can be enabled/disabled as part of a shoot's spec.

Additionally, when a minor Kubernetes or OS version expires, Gardener will force-update the shoot to the next supported version.

Turning on AutoUpdate for a shoot may be convenient but comes at the risk of potentially unwanted changes. While it is possible to revert an OS version update, updates to the Kubernetes version are a one-way operation and cannot be reverted.
...
![node-draining](/docs/getting-started/images/node-draining.gif)

Typically, nodes are drained when:

- There is an update of the OS / Kubernetes minor version
- An Operator cordons and drains a node
...
![node-draining-pdb](/docs/getting-started/images/node-draining-pdb.gif)
...
Pods consume resources and, of course, there are only limited resources available on a single node. Setting requests will make the scheduling much better, as the scheduler has more information available.
...
- Set memory limits and monitor OOM kills of workloads (typically detectable by container status exit code 137 and corresponding events). This will decrease the likelihood of OOM situations on the node itself. However, for critical workloads it might be better to have uncapped growth and rather risk a node going OOM.
...
![user-webhook](/docs/getting-started/images/user-webhook.gif)
...
- Is the pod definition actually valid?
...
**MutatingWebhookConfiguration**: change a resource before it is actually stored in etcd (that is, before any other controller acts upon it)

Both `ValidatingWebhookConfiguration` and `MutatingWebhookConfiguration` resources
...
![user-webhook-fail](/docs/getting-started/images/user-webhook-fail.gif)
...
If the webhook server is not responding in time with a valid answer, the request will time out and the failure policy will be invoked. Depending on the scope of the webhook, frequent failures may cause downtime for applications.
...
- The pod is processing too slowly (e.g., because there are too many requests)

### Timeout

![timeout](/docs/getting-started/images/timeout.webp)
...
> [!NOTE]
> Webhooks (esp. mutating) may be called sequentially and thus their individual timeouts might be added up. Even with a `failurePolicy=ignore` the timeout will stop the request.
...
For more information, see [Shoot Status](/docs/gardener/shoot/shoot_status/#constraints).

## Conversion Webhooks

### Who installs a conversion webhook?

If you have written your own `CustomResourceDefinition` (CRD) and made a version upgrade, you will also have consciously written and deployed the conversion webhook.
...
![conversion-webhook-crd](/docs/getting-started/images/conversion-webhook-crd.webp)
