...
github_subdir: geps/0019-observability-stack-operator-migration
params:
  github_branch: main
path_base_for_github_subdir:
  from: content/docs/proposals/0019-observability-stack-operator-migration/README.md
  to: README.md
...
<!--
   ...
   └───────────│────────────────────────────────────┘
               ▼               
   https://github.com/gardener/enhancements/blob/main/geps/0019-observability-stack-operator-migration/README.md
-->
...
introducing Kubernetes operators such as the [prometheus-operator](https://github.com/prometheus-operator/prometheus-operator) for monitoring stack
and [fluent-bit-operator](https://github.com/fluent/fluent-operator) for the logging stack.
...
1. [Prometheus Custom Resources](https://github.com/prometheus-operator/prometheus-operator#customresourcedefinitions) provided by the prometheus-operator.
1. [Fluent-bit Custom Resources](https://github.com/fluent/fluent-operator/tree/master/apis/fluentbit/v1alpha2) provided by the fluent-bit-operator.
...
[prometheus-operator](https://github.com/prometheus-operator/prometheus-operator) and [fluent-bit-operator](https://github.com/fluent/fluent-operator). The current extension contract is described
[here](https://github.com/gardener/gardener/tree/master/docs/extensions/logging-and-monitoring.md). This document aims to define a new contract.
...
these stacks to use the [prometheus-operator](https://github.com/prometheus-operator/prometheus-operator) and [fluent-bit-operator](https://github.com/fluent/fluent-operator).
The proposal is lined out below:

### API

Use the [API](https://github.com/prometheus-operator/prometheus-operator/tree/main/pkg/apis/monitoring) provided by the [prometheus-operator](https://github.com/prometheus-operator/prometheus-operator) and create Go structs.

### Prometheus Operator CRDs

Deploy the [prometheus-operator](https://github.com/prometheus-operator/prometheus-operator) and its CRDs. These components can be deployed
...
the [prometheus-operator](https://github.com/prometheus-operator/prometheus-operator) and the operator itself can be found
[here](https://github.com/prometheus-operator/prometheus-operator/blob/main/bundle.yaml). This step is a prerequisite for all other steps.

### Shoot Monitoring

Gardener will create a monitoring stack similar to the current one with the
[prometheus-operator](https://github.com/prometheus-operator/prometheus-operator) custom resources.

1. Most of the shoot monitoring is deployed via this
   [chart](https://github.com/gardener/gardener/tree/master/charts/seed-monitoring/charts). The goal is to create a similar stack, but not
   necessarily with feature parity, using the [prometheus-operator](https://github.com/prometheus-operator/prometheus-operator).
...
1. Prometheus needs to discover targets running in the shoot cluster.
   ...
   ([example](https://github.com/gardener/gardener/blob/0f4d22270927e2aee8b821f858fb76162ccd8a86/charts/seed-monitoring/charts/core/charts/prometheus/templates/config.yaml#L311)). This is currently not possible with the
   prometheus-operator, but there is an open [issue](https://github.com/prometheus-operator/prometheus-operator/issues/4828).
   
   - Preferred approach: A second Prometheus can be created that is running
     in [agent mode](https://prometheus.io/blog/2021/11/16/agent/). This Prometheus can also be deployed/managed by the
     [prometheus-operator](https://github.com/prometheus-operator/prometheus-operator). The agent Prometheus can be configured to use
   ...
   - Alternative: Use [additional scrape config](https://github.com/prometheus-operator/prometheus-operator/blob/main/Documentation/additional-scrape-config.md). In this case, the
     Prometheus config snippet is put into a secret and the
     [prometheus-operator](https://github.com/prometheus-operator/prometheus-operator) will append it to the config. The downside here is
...
1. Deploy an optional [alertmanager](https://github.com/gardener/gardener/tree/master/charts/seed-monitoring/charts/alertmanager) that is deployed
   whenever the end-user [specifies](https://github.com/gardener/gardener/tree/master/docs/monitoring/alerting.md#alerting-for-users) alerting.
...
1. Health checks - The gardenlet periodically checks the status of the
   ...
   [prometheus-operator](https://github.com/prometheus-operator/prometheus-operator) will create `StatefulSets` with the same labelset
...
also be provided using the [prometheus-operator](https://github.com/prometheus-operator/prometheus-operator). The steps for this are very
similar to the shoot monitoring.

- Replace the Prometheis and their configuration.

- Replace the [alertmanager](https://github.com/gardener/gardener/blob/0f4d22270927e2aee8b821f858fb76162ccd8a86/charts/seed-bootstrap/templates/alertmanager/alertmanager.yaml) and its configuration.
...
- In general, components should bring their own monitoring configuration.
  ...
  [gardener-resource-manager](https://github.com/gardener/gardener/blob/eec37223cb90475ec3e023136a7d5ba28ad48f0d/pkg/component/resourcemanager/monitoring.go). This configuration is then appended to the
...
- Adding meta monitoring for the extensions: If the extensions need to be
  ...
  Resources](https://github.com/prometheus-operator/prometheus-operator#customresourcedefinitions).
  - Currently the contract between extensions and gardener is that
    ...
    defined [here](https://github.com/gardener/gardener/blob/201673c1f8a356a63b21505ca9c7f6efe725bd48/charts/seed-bootstrap/charts/monitoring/templates/config.yaml#L14-L36). This is something that we can still
    support with a `PodMonitor` that will scrape any pod in a specified
    namespace with these labels.

### Grafana Sidecar

Add a [sidecar](https://github.com/kiwigrid/k8s-sidecar) to Grafana that will pickup dashboards and provision them. Each dashboard gets its own configmap.
...
- Grafana in the control plane
  ...
  - Contract between dashboards in configmaps and the Grafana sidecar.
    ...
    - The Grafana [sidecar](https://github.com/kiwigrid/k8s-sidecar) must be [configured](https://github.com/kiwigrid/k8s-sidecar#configuration-environment-variables) with:
...
[fluent-bit-resource](https://github.com/fluent/fluent-operator/blob/4b984f7c9440119ebacc2c31b8ec376bfd0d63c4/apis/fluentbit/v1alpha2/fluentbit_types.go#L31) carries the usual Kubernetes workload properties,
such as resource definitions, node selectors, pod/node affinity, and so on. The sole purpose
is to construct the [fluent-bit-daemonset](https://github.com/fluent/fluent-operator/blob/4b984f7c9440119ebacc2c31b8ec376bfd0d63c4/pkg/operator/daemonset.go#L13) spec managing the fluent-bit application instances
...
In this example, we have a fluent bit [filter-plugin](https://docs.fluentbit.io/manual/pipeline/filters/parser) of type parser and the corresponding [regex-parser](https://docs.fluentbit.io/manual/pipeline/parsers/regular-expression). In the Gardener context, these configuration resources are supplied by the core components and extensions.

### BYOLC (Bring Your Own Logging Configuration)

Since fluent-bit uses [input-tail](https://docs.fluentbit.io/manual/pipeline/inputs/tail) plugin and reads any container output under `/var/log/pods`, it processes logs outputs of all workloads. In this case, any workload may bring its own set of filters and parsers if needed, using the declarative APIs supported by the operator.

### Migration

#### Prometheus Operator

1. Deploy the [prometheus-operator](https://github.com/prometheus-operator/prometheus-operator) and its custom resources.
...
1. Any extension or monitoring configuration that is not migrated to the [prometheus-operator](https://github.com/prometheus-operator/prometheus-operator) right away will be collected and added to an `additionalScrapeConfig`. Once all extensions and components have migrated, this can be dropped.
