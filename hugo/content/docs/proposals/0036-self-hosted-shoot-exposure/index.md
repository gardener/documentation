...
github_subdir: geps/0036-self-hosted-shoot-exposure
params:
  github_branch: main
path_base_for_github_subdir:
  from: content/docs/proposals/0036-self-hosted-shoot-exposure/README.md
  to: README.md
...
<!--
   ...
   └───────────│────────────────────────────────────┘
               ▼               
   https://github.com/gardener/enhancements/blob/main/geps/0036-self-hosted-shoot-exposure/README.md
-->
...
This proposal introduces a standardized mechanism for exposing the API server of self-hosted shoot clusters with managed infrastructure (see [GEP-28](/docs/proposals/0028-self-hosted-shoot-clusters/#managed-infrastructure)), e.g., using a load balancer of the underlying infrastructure provider or other strategies.
...
The DNS record points to the load balancer of an istio ingress gateway of the hosting seed cluster (see [GEP-08](/docs/proposals/0008-shoot-apiserver-via-sni/)).
...
If the new `exposure.extension` field is set, `gardenadm init` (for initial bootstrapping) or the `gardenlet` (after [connecting the shoot to a garden](/docs/proposals/0028-self-hosted-shoot-clusters/#gardenadm-connect)) creates/updates a `SelfHostedShootExposure` object in the `kube-system` namespace (similar to the other self-hosted shoot extension objects).
...
The `spec` includes the default set of fields included in all extension resources like `type` and `providerConfig` (see [GEP-01](/docs/proposals/0001-gardener-extensibility/)).
...
In [provider-local](https://github.com/gardener/gardener/tree/master/docs/extensions/provider-local.md), the `SelfHostedShootExposure` controller can create a `Service` of type `LoadBalancer` in underlying kind cluster and configure it to forward traffic to the control plane nodes.
