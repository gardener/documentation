...
github_subdir: geps/0030-apiserver-proxy
params:
  github_branch: main
path_base_for_github_subdir:
  from: content/docs/proposals/0030-apiserver-proxy/README.md
  to: README.md
...
<!--
   ...
   └───────────│────────────────────────────────────┘
               ▼               
   https://github.com/gardener/enhancements/blob/main/geps/0030-apiserver-proxy/README.md
-->
...
## Summary
[](/docs/proposals/0008-shoot-apiserver-via-sni/)
This proposal reworks the API server proxy (originally introduced in [GEP-08](/docs/proposals/0008-shoot-apiserver-via-sni/)) to use [HTTP CONNECT requests](https://datatracker.ietf.org/doc/html/rfc7231#section-4.3.6) (i.e., HTTP proxy) instead of the [proxy protocol](https://www.haproxy.org/download/3.2/doc/proxy-protocol.txt) for connecting in-cluster clients on the shoot side to the corresponding API server on the seed side.
With this, the API server proxy uses the same network infrastructure and protocol to connect to the shoot control plane as the shoot's VPN client (see [GEP-14](/docs/proposals/0014-reversed-cluster-vpn/)).
...
Since [GEP-08](/docs/proposals/0008-shoot-apiserver-via-sni/) introduced shared LoadBalancers for shoot control planes on the seed side, clients need to indicate which control plane they want to connect to through the LoadBalancer.
...
1. The Envoy process in the API server proxy pod on the same node sends an HTTP CONNECT request to the API server domain of the shoot, i.e., to the ingress gateway on the corresponding seed.
1. The ingress gateway discards the target from the HTTP request line and opens a TCP connection to the upstream cluster indicated by the `X-Gardener-Destination` header, i.e., in-cluster service of the shoot cluster.
1. The TLS payload is proxied from the in-cluster client via the API server proxy and the ingress gateway to the shoot API server.
...
1. It then matches the CONNECT type and the content of the `X-Gardener-Destination` header. It only matches requests that indicate a valid destination (shoot API server or VPN server).
1. If the request is matched, the gateway opens a connection to the specified destination and proxies the raw TCP payload from the client over this connection.
...
1. Reconfigure the API server proxy as described in [this section](#reconfiguring-the-api-server-proxy).
...
1. Introduce a new feature gate `RemoveAPIServerProxyLegacyPort` to gardenlet.
...
1. Graduate the `RemoveAPIServerProxyLegacyPort` feature gate to GA and lock to `True`.
...
1. Graduate the `UseUnifiedHTTPProxyPort` feature gate.
...
1. Introduce a new feature gate `RemoveHTTPProxyLegacyPort` to gardenlet.
...
1. Graduate the `RemoveHTTPProxyLegacyPort` feature gate to GA and lock to `True`.
...
![Current Architecture](/docs/proposals/0030-apiserver-proxy/30-current-architecture.png)
...
![Proposed Architecture](/docs/proposals/0030-apiserver-proxy/30-proposed-architecture.png)
