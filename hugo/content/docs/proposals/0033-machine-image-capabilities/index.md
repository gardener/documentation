...
github_subdir: geps/0033-machine-image-capabilities
params:
  github_branch: main
path_base_for_github_subdir:
  from: content/docs/proposals/0033-machine-image-capabilities/README.md
  to: README.md
...
<!--
   ...
   └───────────│────────────────────────────────────┘
               ▼               
   https://github.com/gardener/enhancements/blob/main/geps/0033-machine-image-capabilities/README.md
-->
...
![](/docs/proposals/0033-machine-image-capabilities/gep-33-screenshot-dashboard.png)
...
![](/docs/proposals/0033-machine-image-capabilities/gep-33-capabilities.drawio.svg)
...
1. In the cloud profile only one OS image version can be in state `classification: supported`.
...
The image versions in `spec.machineImages.versions` will be extended with `capabilityFlavors` - an array of capabilities maps, one entry for each image reference of an image version in the cloud provider.

The architecture is also added to the image version capability flavors. This is required as the architecture is a capability itself.
...
All capability combination provided in the `spec.machineImages.versions.capabilityFlavors` must have a matching entry in the `spec.providerConfig.machineImages.versions` to ensure that the provider extension can select the correct image reference.
...
1. If the most preferred value is identical for all capabilities, the next most preferred values are compared, continuing this process until a selection is made.
