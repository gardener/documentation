...
github_subdir: geps/0025-namespaced-cloudprofiles
params:
  github_branch: main
path_base_for_github_subdir:
  from: content/docs/proposals/0025-namespaced-cloudprofiles/README.md
  to: README.md
...
<!--
   ...
   └───────────│────────────────────────────────────┘
               ▼               
   https://github.com/gardener/enhancements/blob/main/geps/0025-namespaced-cloudprofiles/README.md
-->
...
[CloudProfiles](/docs/gardener/concepts/apiserver/#cloudprofiles) are non-namespaced objects that are managed centrally by Gardener operators. They usually contain not only global configuration, but options that are relevant to certain projects only (e.g. special machine types). This increases the operation burden for operators and clutters `CloudProfile` objects. On the other hand, users are blocked until the requested special configuration is rolled out to the desired landscapes.
...
1. Need to use different volume types than the ones present in the `CloudProfile`.
1. Extending the expiration date of Kubernetes versions. Given that the `CloudProfile` is a cluster-scoped resource, it is currently not possible to extend the expiration date for shoots in one project but only centrally for all projects.
1. Extending the expiration date for machine images. For the same reasons as extending the expiration date of the Kubernetes versions.
...
1. Migrate `Shoot`s `cloudProfileName` to the `cloudProfile` field but keep `cloudProfileName` in the API for now
1. Enable `NamespacedCloudProfiles` in Gardener
...
Instead of specifying a `NamespacedCloudProfile` resource, an end user could be allowed to enter arbitrary names in fields such as `machineTypes`. The entry would then, if the user enters a wrong value, throw an error when provisioning the resource at the cloud provider. However, this approach does not seem feasible as the metadata that is specified in a `CloudProfile` like the number of CPUs and amount of RAM is used in the trial clusters (see [Shoot Quotas](/docs/gardener/concepts/apiserver/#shoot-quotas)) to validate quotas and to enable the scale-from-zero feature. Therefore, an exception would need to be developed for this specific, and possibly other, use cases.
