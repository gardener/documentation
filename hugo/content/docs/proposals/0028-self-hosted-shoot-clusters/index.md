...
github_subdir: geps/0028-self-hosted-shoot-clusters
params:
  github_branch: main
path_base_for_github_subdir:
  from: content/docs/proposals/0028-self-hosted-shoot-clusters/README.md
  to: README.md
...
<!--
   ...
   └───────────│────────────────────────────────────┘
               ▼               
   https://github.com/gardener/enhancements/blob/main/geps/0028-self-hosted-shoot-clusters/README.md
-->
...
![Overview](/docs/proposals/0028-self-hosted-shoot-clusters/28-overview.png)
...
In addition, Gardener itself requires to run in a conformant Kubernetes cluster satisfying the [minimum Kubernetes version requirements](https://github.com/gardener/gardener/tree/master/docs/usage/shoot-operations/supported_k8s_versions.md#garden-clusters), but so far there was no way of setting up or managing this initial cluster via Gardener itself.
...
<img src="https://github.com/gardener/gardener/raw/master/logo/gardenadm-large.png" alt="gardenadm" width="100"/>

In order to reduce cognitive load, we'll align the user experience of this new tool with what [`kubeadm`](https://kubernetes.io/docs/reference/setup-tools/kubeadm/) provides (note that `gardenadm` is not meant to replace `kubeadm` - we just want to reduce the entry barrier for people already familiar with `kubeadm`, [similar to how we have aligned the Gardener architecture with Kubernetes concepts](https://github.com/gardener/gardener/tree/master/README.md#gardener)).
After a cluster has been set up with `gardenadm`, the next step is to deploy a stripped-down version of [`gardenlet`](https://github.com/gardener/gardener/tree/master/docs/concepts/gardenlet.md) into it.
...
1. The downloaded configuration and a `Shoot` manifest are provided to the `gardenadm init` command.
   This will bootstrap the first control plane node.
1. Optionally, additional control plane nodes can be joined.
...
1. Optionally, worker nodes can be joined.
...
1. Once above steps are completed, the user runs `gardenadm connect` to deploy a `gardenlet` into the cluster which connects to an existing Gardener system for further cluster management.
...
1. Generate certificate authorities, certificates and keys using the [secrets manager](https://github.com/gardener/gardener/tree/master/docs/development/secrets_management.md).
1. Generate [`OperatingSystemConfig` resource](https://github.com/gardener/gardener/tree/master/docs/extensions/resources/operatingsystemconfig.md) (in memory) containing `systemd` units and files.
...
1. Reconcile the `OperatingSystemConfig` resource using the controller logic of [`gardener-node-agent`](https://github.com/gardener/gardener/tree/master/docs/concepts/node-agent.md).
1. `kubelet` starts and runs the static pods for the minimal standard Kubernetes control plane (`etcd`, `kube-apiserver`, `kube-controller-manager` and `kube-scheduler`).
1. Deploy [`gardener-resource-manager`](https://github.com/gardener/gardener/tree/master/docs/concepts/resource-manager.md) in [host network mode](#host-network-vs-pod-network) into the `kube-system` namespace.
1. Apply the corresponding provider and networking extensions in [host network mode](#host-network-vs-pod-network).
1. Deploy `kube-proxy` and CoreDNS for service routing and domain name resolution.
1. Create a [`Network` resource](https://github.com/gardener/gardener/tree/master/docs/extensions/resources/network.md) and apply it to generate `ManagedResource` associated with the pod network.
1. Apply the corresponding provider and networking extensions in [pod network mode](#host-network-vs-pod-network).
   Also deploy the operating system config extensions.
1. Redeploy `gardener-resource-manager` in [pod network mode](#host-network-vs-pod-network).
1. Create the [`OperatingSystemConfig` resource](https://github.com/gardener/gardener/tree/master/docs/extensions/resources/operatingsystemconfig.md) now in the cluster and activate the `gardener-node-agent` process via its `systemd` unit.
1. Apply [`ControlPlane` resource](https://github.com/gardener/gardener/tree/master/docs/extensions/resources/controlplane.md).
...
However, some extensions might want to mutate them [via webhooks](https://github.com/gardener/gardener/tree/master/docs/extensions/controlplane-webhooks.md).
...
This command instruments `gardener-node-agent` reconciliation logic to write the `systemd` units and needed files for the [`gardener-node-init`](https://github.com/gardener/gardener/tree/master/docs/concepts/node-agent.md#installation-and-bootstrapping) component onto the disk of the machine.
...
> [!NOTE]
> When there is no existing Gardener system yet (for example, because the created self-hosted shoot cluster is the initial cluster), then [`gardener-operator`](https://github.com/gardener/gardener/tree/master/docs/concepts/operator.md) must be deployed first, and the [`Garden`](https://github.com/gardener/gardener/tree/master/example/operator/20-garden.yaml) must be created, before the `gardenlet` can be deployed.
> 
...
It is important that the [version skew](https://github.com/gardener/gardener/tree/master/docs/deployment/version_skew_policy.md) is always respected.
...
- [Managed Infrastructure](#managed-infrastructure), meaning that there is programmable infrastructure available where we can leverage [provider extensions](https://github.com/gardener/gardener/tree/master/docs/extensions/README.md#infrastructure-provider) and [`machine-controller-manager`](https://github.com/gardener/machine-controller-manager) in order to manage the network setup and the machines.
...
Since machine management is out of scope, upgrades requiring to roll out the nodes (e.g., Kubernetes version upgrades, operating system upgrades, CA rotation, [see more](/docs/gardener/shoot-operations/shoot_updates/#rolling-update-triggers)) must be performed in-place.
...
In this scenario, we can leverage the existing [`extensions.gardener.cloud/v1alpha1.Infrastructure` API](https://github.com/gardener/gardener/tree/master/docs/extensions/resources/infrastructure.md) in order to let the respective provider extension generate the necessary network resources (VPCs, subnets, etc., whatever is needed for the respective infrastructure).
...
1. Deploy the required provider extension.
1. Apply the `Infrastructure` resource to create the virtual network.
1. Deploy `machine-controller-manager` to be able to create the machines.
1. Apply the `Worker` resource to create the control plane nodes.
1. Wait for the machines to be created.
1. Stop `machine-controller-manager` to prevent deletion of the machines as they will not become ready since there is no Kubernetes cluster yet to join.
1. Create a `Bastion` resource to be able to access the machines and execute `gardenadm init` (can be later optimized via `cloud-init` through a cloud config that invokes `gardenadm init` directly when creating the machines).
...
As mentioned in the corresponding section, the [version skew](https://github.com/gardener/gardener/tree/master/docs/deployment/version_skew_policy.md) plays an important role in this context, determining the maximum time the self-hosted shoot cluster can be disconnected from Gardener.
...
- The `apiserver-proxy` (see [GEP-11](/docs/proposals/0011-apiserver-network-proxy/) for details) is used to forward requests from the data plane directed at `kubernetes.default.svc.cluster.local` to the control plane.
