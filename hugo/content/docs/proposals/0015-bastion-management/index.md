...
github_subdir: geps/0015-bastion-management
params:
  github_branch: main
path_base_for_github_subdir:
  from: content/docs/proposals/0015-bastion-management/README.md
  to: README.md
...
<!--
   ...
   └───────────│────────────────────────────────────┘
               ▼               
   https://github.com/gardener/enhancements/blob/main/geps/0015-bastion-management/README.md
-->
...
- Gardener (RBAC)
  - The project `admin` role should be extended to allow CRUD operations on the `Bastion` resource. The `gardener.cloud:system:project-member-aggregation` `ClusterRole` needs to be [updated accordingly](https://github.com/gardener/gardener/tree/master/charts/gardener/controlplane/charts/application/templates/rbac-user.yaml)
...
0. User/`gardenctlv2` creates the `Bastion` resource in the garden cluster (see [resource example](#resource-example) below).
...
0. GAPI Admission Plugin for the `Bastion` resource in the garden cluster.
...
0. Gardener extension provider `<infra>` / Bastion Controller on Seed:
...
### Rotation Proposal
- `gardeneruser` original user data [component](https://github.com/gardener/gardener/tree/master/pkg/component/extensions/operatingsystemconfig/original/components/gardeneruser):
...
- On rotation trigger:
  ...
  - As usual (for more details, see [Contract: OperatingSystemConfig Resource](https://github.com/gardener/gardener/tree/master/docs/extensions/resources/operatingsystemconfig.md)):
    - Once the `cloud-config-<X>` secret in the `kube-system` namespace of the shoot cluster is updated, it will be picked up by the [`downloader` script](https://github.com/gardener/gardener/tree/master/pkg/component/extensions/operatingsystemconfig/downloader/templates/scripts/download-cloud-config.tpl.sh) (checks every 30s for updates).
    - The `downloader` runs the ["execution" script](https://github.com/gardener/gardener/tree/master/pkg/component/extensions/operatingsystemconfig/executor/templates/scripts/execute-cloud-config.tpl.sh) from the `cloud-config-<X>` secret.
    ...
    - Running the [original user data](https://github.com/gardener/gardener/tree/master/pkg/component/extensions/operatingsystemconfig/original) script will also run the `gardeneruser` component, where the `authorized_keys` file will be updated.
