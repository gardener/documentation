...
github_subdir: geps/0017-shoot-control-plane-migration-bad-case
params:
  github_branch: main
path_base_for_github_subdir:
  from: content/docs/proposals/0017-shoot-control-plane-migration-bad-case/README.md
  to: README.md
...
<!--
   ...
   └───────────│────────────────────────────────────┘
               ▼               
   https://github.com/gardener/enhancements/blob/main/geps/0017-shoot-control-plane-migration-bad-case/README.md
-->


# GEP-0017: Shoot Control Plane Migration "Bad Case" Scenario

The [migration flow](/docs/proposals/0007-shoot-control-plane-migration/#migration-workflow) described as part of [GEP-7](/docs/proposals/0007-shoot-control-plane-migration/) can only be executed if both the Garden cluster and source seed cluster are healthy, and the `gardenlet` in the source seed cluster can connect to the Garden cluster. In this case, the `gardenlet` can directly scale down the shoot's control plane in the source seed, after checking the `spec.seedName` field.
...
1. The special "copy" version of the `etcd-main` Etcd resource is deleted from the source seed, and as a result the `etcd-copy` Job is also deleted by `etcd-druid`.

1. The additional "source" backup entry referencing the source seed backup container is deleted from the Garden cluster and the destination seed. As a result, its corresponding `source-etcd-backup` secret is also deleted from the destination seed.

1. From this point, the reconciliation flow proceeds as already described in [GEP-7](/docs/proposals/0007-shoot-control-plane-migration/). This is safe, since the source seed cluster is no longer able to interfere with the shoot.
