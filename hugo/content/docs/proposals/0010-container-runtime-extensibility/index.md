...
github_subdir: geps/0010-container-runtime-extensibility
params:
  github_branch: main
path_base_for_github_subdir:
  from: content/docs/proposals/0010-container-runtime-extensibility/README.md
  to: README.md
...
<!--
   ...
   └───────────│────────────────────────────────────┘
               ▼               
   https://github.com/gardener/enhancements/blob/main/geps/0010-container-runtime-extensibility/README.md
-->
...
1. Change the nodes container runtime to work with CRI and ContainerD (Only if specified in the shoot spec):
   ...
   1. ContainerD and Docker configurations are different for each OS. To make sure the default configurations above work well in each worker machine, each OS extension would be responsible to configure them during the reconciliation of theOperatingSystemConfig:
      1. os-ubuntu:
   ...
   1. To rotate the ContainerD (CRI) logs we will activate the kubelet feature flag: `CRIContainerLogRotation=true`.
   1. Docker monitor service will be replaced with equivalent ContainerD monitor service.

1. Validate workers additional runtime configurations:
   1. Disallow additional runtimes with shoots < 1.14
   1. kata-container validation: Machine type support nested virtualization.
1. Add support for each additional container runtime in the cluster.
   1. In order to install each additional available runtime in the cluster we should:
      ...
      1. Apply the relevant RuntimeClass to the cluster.
   
   1. The installation above should be done by a new kind of extension: ContainerRuntime resource. For each container runtime type (Kata-container/gvisor), a dedicate extension controller will be created.
      ...
      1. Each runtime extension controller will be responsible to reconcile its RuntimeContainer resource type.
         ...
         1. The runtime extension controller from specific type should apply a chart which responsible for the installation of the runtime container in the cluster:
            ...
            1. RuntimeClasses in the cluster to support the runtime class. For example:
               ```yaml
               ...
               handler: runsc
               ```
         1. Update the status of the relevant RuntimeContainer resource to succeeded.
