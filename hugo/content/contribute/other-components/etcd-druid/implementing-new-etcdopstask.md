...
path_base_for_github_subdir:
  from: >-
    content/contribute/other-components/etcd-druid/implementing-new-etcdopstask.md
  to: implementing-new-etcdopstask.md
persona: Developers
...
1. **Implement the Task Handler**:
   ...
   > 1) The `Execute` method is called repeatedly in case of timeouts/transient errors until the task execution is successful or a non-retryable error occurs. Once this is done the task moves to `Succeeded` or `Failed` state respectively.
   > 1) The `Cleanup` method is called when the task reaches a terminal state (Succeeded/Failed/Rejected) to clean up any resources created during the task execution. This can be a no-op if there are no resources to clean up. (Refer the [on-demand-snapshot handler implementation](https://github.com/gardener/etcd-druid/tree/master/internal/controller/etcdopstask/handler/ondemandsnapshot/ondemandsnapshot.go) for reference).
   > 1) Refer the etcdopstask [api](https://gitthub.com/gardener/etcd-druid/tree/master/api/core/v1alpha1/etcdopstask.go) for details regarding the etcdopstask state transitions and status fields.
