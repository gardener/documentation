...
path_base_for_github_subdir:
  from: content/contribute/other-components/etcd-druid/running-e2e-tests.md
  to: running-e2e-tests.md
persona: Developers
...
<!--
   ...
   └───────────│────────────────────────────────────┘
               ▼               
   https://github.com/gardener/etcd-druid/blob/master/docs/development/running-e2e-tests.md
-->
...
1. Install Etcd-Druid into the cluster, configured for e2e testing.
1. Run the full e2e test suite (see [make test-e2e](#running-e2e-tests-against-an-existing-cluster) below).
1. Cleanup e2e test resources, based on the `RETAIN_TEST_ARTIFACTS` flag.
1. Tear down the KinD cluster, based on the `RETAIN_KIND_CLUSTER` flag.
...
> Check the [etcd-druid deployment documentation](/docs/other-components/etcd-druid/deployment/getting-started-locally/getting-started-locally/#02-setting-up-etcd-druid) for more details.
...
Port mappings for the services deployed for these emulators are defined in the [KinD cluster configuration file](https://github.com/gardener/etcd-druid/blob/master/hack/kind-up.sh), which allows developers to use the storage providers' respective CLIs to access and inspect the storage buckets created when deploying these emulators.
...
Please refer to the [Localstack S3 emulator documentation](/docs/other-components/etcd-druid/deployment/getting-started-locally/manage-s3-emulator/) for detailed instructions on deploying Localstack.

**Setup steps for e2e tests:**

1. Deploy Localstack: `make deploy-localstack`
1. Create the test bucket `default.bkp`:
   ```bash
...
Please refer to the [Azurite emulator documentation](/docs/other-components/etcd-druid/deployment/getting-started-locally/manage-azurite-emulator/) for detailed instructions on deploying Azurite.

**Setup steps for e2e tests:**

1. Deploy Azurite: `make deploy-azurite`
1. Create the test container `default.bkp`:
...
Please refer to the [fake-gcs-server documentation](/docs/other-components/etcd-druid/deployment/getting-started-locally/manage-gcs-emulator/) for detailed instructions on deploying fake-gcs-server.
...
1. Create a dummy service account JSON file (FakeGCS doesn't validate credentials):
