...
path_base_for_github_subdir:
  from: content/contribute/gardener/secrets_management.md
  to: secrets_management.md
persona: Developers
...
<!--
   ...
   └───────────│────────────────────────────────────┘
               ▼               
   https://github.com/gardener/gardener/blob/master/docs/development/secrets_management.md
-->
...
These requirements can be covered by using the `SecretsManager` package maintained in [`pkg/utils/secrets/manager`](https://github.com/gardener/gardener/tree/master/pkg/utils/secrets/manager).
It is built on top of the `ConfigInterface` and `DataInterface` interfaces part of [`pkg/utils/secrets`](https://github.com/gardener/gardener/tree/master/pkg/utils/secrets) and provides the following functions:
...
1. `gardenlet` deploys a server (`etcd`) in one step, and a client (`kube-apiserver`) in a subsequent step. In this case, the default behaviour should apply (server certificate should be signed by old/existing CA).
...
1. `gardenlet` deploys a server (`etcd`) in one step, and a client (`kube-apiserver`) in a subsequent step. In this case, the default behaviour should apply (client certificate should be signed by new/current CA).
