---
description: Developer documentation for the Gardener web UI
github_repo: 'https://github.com/gardener/dashboard'
github_subdir: .
params:
  github_branch: master
path_base_for_github_subdir:
  from: content/contribute/dashboard/_index.md
  to: README.md
title: Dashboard
weight: 60
prev: false
next: false
---
<!-- BANNER:MANAGED -->
<!--
   █▀▀ ▀█▀ █▀█ █▀█
   ▀▀█  █  █ █ █▀▀
   ▀▀▀  ▀  ▀▀▀ ▀

   ┌────────────────────────────────────────────────┐
   │  MANAGED FILE — aggregated from upstream       │
   │                                                │
   │  Editing here is pointless: The nightly        │
   │  aggregation run overwrites this file.         │
   │                                                │
   │  Open a PR against the source instead: ─────┐  │
   │                          ┌──┐    ┌──────────┘  │
   │                          └──│────┘             │
   │           ┌─────────────────┘                  │
   └───────────│────────────────────────────────────┘
               ▼               
   https://github.com/gardener/dashboard/blob/master/./README.md
-->


# Gardener Dashboard
[![REUSE status](https://api.reuse.software/badge/github.com/gardener/dashboard)](https://api.reuse.software/info/github.com/gardener/dashboard)

![](/docs/dashboard/logo_gardener_dashboard.png)

[![CI Build status](https://github.com/gardener/dashboard/actions/workflows/non-release.yaml/badge.svg?branch=master)](https://github.com/gardener/dashboard/actions/workflows/non-release.yaml?query=branch%3Amaster)
[![Slack workspace](https://img.shields.io/badge/Slack-Gardener%20Project-brightgreen.svg?logo=slack)](https://gardener-cloud.slack.com/)

## Demo

<img src="/docs/dashboard/images/dashboard-demo.gif" alt="Gardener Demo"/>

## Documentation

[Gardener Dashboard Documentation](https://github.com/gardener/dashboard/blob/master/docs/README.md)

## License
[Apache License 2.0](https://github.com/gardener/dashboard/blob/master/LICENSES/Apache-2.0.txt)

Copyright 2020 The Gardener Authors
