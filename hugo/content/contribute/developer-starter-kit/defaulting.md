...
<!--
   ...
   └───────────│────────────────────────────────────┘
               ▼               
   https://github.com/gardener/gardener/blob/master/docs/development/defaulting.md
-->
...
- Every kubernetes type should have a dedicated `defaults_*.go` file. For instance, if you have a [`Shoot`](https://github.com/gardener/gardener/blob/master/pkg/apis/core/types_shoot.go) type, there should be a corresponding [`defaults_shoot.go`](https://github.com/gardener/gardener/blob/master/pkg/apis/core/v1beta1/defaults_shoot.go) file containing all defaulting logic for that type.
- If there is only one type under an api group then we can just have `types.go` and a corresponding `defaults.go`. For instance, `resourcemanager` api has only one [`types.go`](https://github.com/gardener/gardener/blob/master/pkg/apis/config/resourcemanager/v1alpha1/types.go), hence in this case only [`defaults.go`](https://github.com/gardener/gardener/blob/master/pkg/apis/config/resourcemanager/v1alpha1/defaults.go) file would suffice.
