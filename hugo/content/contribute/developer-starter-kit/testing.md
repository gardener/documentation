...
path_base_for_github_subdir:
  from: content/contribute/developer-starter-kit/testing.md
  to: testing.md
title: Testing
weight: 40
prev: false
...
<!--
   ...
   └───────────│────────────────────────────────────┘
               ▼               
   https://github.com/gardener/gardener/blob/master/docs/development/testing.md
-->
...
- Prefer `DescribeTable`/`Entry` whenever the test code is identical across cases and only the input parameters and expectations differ: [example test](https://github.com/gardener/gardener/blob/33b3af3e786b6c57294c6c99a062c0fa6cef36a5/pkg/api/core/helper/seed_test.go#L143-L152)
  - `DescribeTable` is the idiomatic Ginkgo way of writing table tests. Hand-rolled `test := func(...)` helpers that wrap an `It` per case duplicate what `DescribeTable` already does, and they need `Offset(1)`/`ExpectWithOffset(1)` to report failures at the right location.
  - Table entries that need to construct their input lazily can pass constructor functions as `Entry` parameters: [example test](https://github.com/gardener/gardener/blob/33b3af3e786b6c57294c6c99a062c0fa6cef36a5/pkg/utils/kubernetes/pod_test.go#L74-L91)
  - It remains fine to use a helper `test()` func when the per-case setup logic is non-trivial and cannot be expressed as `Entry` parameters.
  - Similarly, prefer `DescribeTableSubtree` over hand-rolled helper funcs when an entire subtree of specs (multiple `It`s) is reused across scenarios (e.g., running the same set of test cases "with workers" vs. "workerless" in e2e tests): [example test](https://github.com/gardener/gardener/blob/33b3af3e786b6c57294c6c99a062c0fa6cef36a5/pkg/nodeagent/controller/operatingsystemconfig/containerd_config_test.go#L133-L209), [docs](https://onsi.github.io/ginkgo/#generating-subtree-tables)
...
If your test suite requires a different control plane configuration (e.g., disabled admission plugins or enabled feature gates), feel free to locally modify the configuration in [`test/start-envtest`](https://github.com/gardener/gardener/tree/master/test/start-envtest) while debugging.
...
It uses a [KinD cluster](https://kind.sigs.k8s.io/) and [skaffold](https://skaffold.dev/) to bootstrap a full installation of Gardener based on the current revision, including [provider-local](/docs/gardener/extensions/provider-local/).
...
For more information, see [Developing Gardener Locally](https://github.com/gardener/gardener/blob/master/docs/development/getting_started_locally.md) and [Deploying Gardener Locally](/contribute/developer-starter-kit/getting_started_locally/).
...
Just like E2E tests, upgrade tests also use a [KinD cluster](https://kind.sigs.k8s.io/) and [skaffold](https://skaffold.dev/) for bootstrapping a full Gardener installation based on the current revision/branch, including [provider-local](/docs/gardener/extensions/provider-local/).
...
Please see [Test Machinery Tests](/contribute/developer-starter-kit/testmachinery_tests/).
