---
aliases:
  - /blog/2025/08/04/announcing-cluster-api-provider-gardener
authors:
...
Specifically, CAPGa implements the Cluster API's provider interfaces to manage Gardener's [Shoot clusters](https://gardener.cloud/docs/getting-started/shoots/) as Cluster API `Cluster` resources. This allows users to provision, update, and delete clusters managed by Gardener via standard Cluster API tooling and workflows.
...
![Image showing the interaction of CAPI and Gardener through CAPGa](/blog/2025/08/static/capi-interaction-gardener-capga.svg)
...
![Illustration: Gardener managed by service team](/blog/2025/08/static/gardener-managed.drawio.svg)
...
![Illustration: Cluster API self-managed](/blog/2025/08/static/user-managed-capi.drawio.svg)
...
![demo](/blog/2025/08/static/demo.gif)
...
The long-term goal is to support CAPGa within a centrally managed *Platform Mesh* built on top of KCP. This approach eliminates the need for users to provision and operate a dedicated Kubernetes cluster solely to host their Cluster API components. At the same time, users can interact directly with the Gardener API through the same unified control plane, enabling flexible and consistent cluster lifecycle management across both interfaces. This scenario is also illustrated in the image below.

![Managing Gardener clusters through Platform Mesh using the Gardener API, Cluster API and KCP](/blog/2025/08/static/platform-mesh.drawio.svg)
