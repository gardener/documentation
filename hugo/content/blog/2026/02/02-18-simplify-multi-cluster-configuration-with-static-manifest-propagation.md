...
aliases:
  - >-
    /blog/2026/02/18/simplify-multi-cluster-configuration-with-static-manifest-propagation
authors:
...
1. It creates or updates a single `ManagedResource` that references all such `Secret`s.
1. The `gardener-resource-manager` in the Shoot then ensures that the manifests contained within these `Secret`s are applied to the Shoot cluster.
...
1. **Create a labeled `Secret`** in the `garden` namespace of your Seed cluster from this file.
   ```bash
