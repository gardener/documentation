...
1. With the `SeamlessOverlaySwitch` feature enabled, the Calico extension detects the requested change but pauses the reconfiguration.
1. It begins monitoring the `NetworkUnavailable` condition on all nodes in the cluster.
1. The extension waits until it has verified that **all** nodes are reporting `NetworkUnavailable=False` with the reason `RouteCreated`.
1. Only once this confirmation is received does the extension proceed with rolling the Calico daemonset to the new non-overlay configuration.
