...
Add the extension to your Shoot's `spec.extensions`:
...
After the Shoot reconciles, verify the `GatewayClass` is accepted:
...
Envoy Gateway provisions a `LoadBalancer` Service for the `Gateway`. Traffic is evenly distributed across matching backend pods, and routing rules — path matching, header-based rules, traffic splitting — are expressed directly on the `HTTPRoute`.
