...
path_base_for_github_subdir:
  from: content/community/hackathons/2024-05.md
  to: 2024-05.md
title: May 2024
weight: -202405
prev: false
...
![Group picture](/community/hackathons/images/2024-05.jpg)
...
**Problem Statement:** Today, there are two development setups for Gardener: The first, most common one is based on Gardener's [control plane Helm chart](https://github.com/gardener/gardener/tree/master/charts/gardener/controlplane) and other [custom manifests](https://github.com/gardener/gardener/tree/master/example/gardener-local/etcd) to bring up Gardener and a seed cluster. The second one is using [`gardener-operator`](/docs/gardener/concepts/operator/) and the [`operator.gardener.cloud/v1alpha1.Garden`](https://github.com/gardener/gardener/blob/master/example/operator/20-garden.yaml) resource to bring up a full-fledged garden cluster. However, this setup does not bring up a seed cluster, hence, creating a `Shoot` is not possible. Generally, it would be better if we could harmonize the scenarios such that we only have to maintain one solution.
...
**Problem Statement:** The [`machine-controller-manager-provider-local`](https://github.com/gardener/machine-controller-manager-provider-local) implementation used by [`gardener-extension-provider-local`](/docs/gardener/extensions/provider-local/) is currently maintained in a different GitHub repository. This makes related maintenance and development tasks more complicated and tedious.
