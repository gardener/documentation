...
We developed agents to support an operator with end-user questions about Gardener or a particular shoot cluster. They use LLMs to plan actions and use powerful tools. The main ones being knowledge-base search, ticket-database search, access to garden/seed/shoot clusters via `bash`/`kubectl`, and the ability to extract metrics, logs, and more. This allows the agent to drill deep and surface insights that would otherwise take an operator a lot of time. Of course, LLMs won’t always draw correct conclusions, but even then the agent gives the operator a head start from which they can continue the investigation.
