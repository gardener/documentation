...
So, create yourself a shoot with modified CIDRs. You can find templates for the shoot manifest in the [gardener/gardener repository](https://github.com/gardener/gardener/tree/master/example). You could, for example, change the CIDRs to this:
...
To solve this problem, edit the original seed and set its `spec.visible` field to `false`. This will prevent Gardener from choosing this seed, so now all shoots created via the dashboard should have their control plane on the new, more secure seed.